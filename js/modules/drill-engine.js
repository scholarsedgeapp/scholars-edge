...
const DrillEngineModule = (() => {
  ...
  // ── ONE-TIME MIGRATION (2026-07-05) ────────────────────── //
  // The RE-KEY content pass replaced the question pools under these codes,
  // so earned levels/mastery no longer reflect what the pools now teach.
  // Soft reset: level and state cleared, masteryHistory preserved.
  (function migrateRekey20260705() {
    try {
      if (Storage.getPath('migrations.rekey_2026_07_05', false)) return;
      var touched = false;
      ['G2','G3','G4','G7','U1','U2','U6','R2','R5','R10'].forEach(function (code) {
        if (Storage.getPath('drillLevels.' + code, 1) > 1
            || Storage.getPath('strategyMastery.' + code, 'not_started') !== 'not_started') {
          touched = true;
        }
        Storage.setPath('drillLevels.' + code, 1);
        Storage.setPath('strategyMastery.' + code, 'not_started');
      });
      // Notice only when real progress was reset — a fresh install has
      // nothing to explain, and the message would read as a bug.
      if (touched) Storage.setPath('migrations.rekey_notice_pending', true);
      Storage.setPath('migrations.rekey_2026_07_05', true);
    } catch (e) {
      console.error('[DrillEngine] rekey migration failed:', e);
    }
  })();
  ...
  // Returns a cloned question with its choices shuffled and the answer index
  // remapped, so the correct answer can't be pattern-matched by position.
  // Shuffles a permutation of indices (not the strings) so duplicate choice
  // text can't corrupt the remap, and never mutates the source bank.
  function shuffleChoices(q) {
    var order      = shuffle(q.c.map(function (_c, i) { return i; }));
    var copy       = {};
    for (var k in q) { if (Object.prototype.hasOwnProperty.call(q, k)) copy[k] = q[k]; }
    copy.c = order.map(function (idx) { return q.c[idx]; });
  ...
  function buildSessionQuestions(code) {
    ...
    return shuffle(pool).slice(0, SESSION_SIZE).map(shuffleChoices);
  }
  ...
  const DE_QUESTIONS = {
    ...
    U1:[
      { p:null, q:"What is the defining feature of a \"trap\" answer choice in Reading?", c:["It reuses passage words in ways the text never states","It's always the longest answer choice","It always appears as Choice A or Choice D","It always contradicts the passage"], a:0, e:"Trap answers recycle passage vocabulary in ways that seem right (you recognize the words) but don't accurately represent what the passage says. They distort, extend, or miscombine.", trap_type:"recycled_language" },
      { p:null, q:"You read a question, then the four choices. Choice A uses technical vocabulary you don't recognize. You should:", c:["Automatically eliminate A — unfamiliar vocabulary signals a trap","Automatically select A — unusual vocabulary signals a sophisticated, correct answer","Skip A and pick from B, C, D","Evaluate A like any other choice — check it against the passage"], a:3, e:"Unfamiliar vocabulary is neither a trap signal nor a correctness signal. The question is always: does this choice accurately reflect what the passage says? Return to the passage and evaluate on evidence.", trap_type:null },
    ],
    U2:[
    ],
    U3:[
      { p:null, q:"After eliminating two choices, you have 45 seconds left. You feel equally uncertain about the two remaining options. You should:", c:["Leave it blank — uncertain guesses hurt more than blanks","Pick the one that uses more words from the passage (familiarity test)","Pick the closest match to your reading of the passage, commit, and move on","Spend 2 more minutes to reach certainty"], a:2, e:"No penalty for guessing. 45 seconds of additional analysis on a 2-choice dilemma is unlikely to change your answer. Pick the best supported choice and move. Committing is a skill.", trap_type:"half_right" },
      { p:null, q:"Which type of question should generally receive MORE than the average time allocation?", c:["Single vocabulary-in-context questions with a short sentence","Single-sentence grammar correction questions","Math questions asking for a simple sum","Questions with long paired passages requiring cross-reference"], a:3, e:"Paired passage cross-reference questions require reading both passages + synthesizing. They deserve above-average time. Short vocabulary or grammar questions can be answered in under 30 seconds.", trap_type:null },
      { p:null, q:"You spend 4 minutes on a hard question, get it right, but run out of time on question 27 and leave it blank. What is the likely outcome compared to spending 90 seconds, guessing, and answering question 27?", c:["The 90-second approach; a blank scores 0 while a guess has value","The 4-minute approach is better because you got a hard one right","Both approaches produce identical expected scores","The 4-minute approach is always better for hard questions"], a:0, e:"Expected value: blank = 0 points. A guess = 0.25 expected points. Hard question = 1 point, but you already got it right in the 90-second scenario too (most of the time). Blanks are the worst outcome.", trap_type:null },
    ],
    U4:[
    ...
    U7:[
      ...
      { p:null, q:"The SAT Reading module has 27 questions and 32 minutes. On average, how many seconds per question is that?", c:["About 45 seconds","About 120 seconds","About 90 seconds","About 71 seconds"], a:3, e:"32 minutes × 60 = 1920 seconds ÷ 27 questions ≈ 71 seconds per question. Passage-based questions require more time; single-sentence grammar questions take less.", trap_type:null },
      { p:null, q:"You just finished question 8 and have 22 minutes left for 19 questions. Are you on track?", c:["No — you should have 25 minutes left by question 8","No — you need at least 30 minutes for 19 questions","Yes — 22 min ÷ 19 ≈ 69 sec/question, on target","Yes, but only if you speed up dramatically"], a:2, e:"22 min ÷ 19 questions ≈ 69 seconds each — nearly exactly the 71-second target. You're on pace.", trap_type:null },
    ],
    ...
    R1:[
      ...
      { p:null, q:"You're eliminating choices. Choice A says \"The author is outraged by the findings.\" The passage says \"The author notes the findings with concern.\" Which elimination trigger applies?", c:["Scope error — \"findings\" vs. \"the topic\"","Extreme language — 'outraged' intensifies 'concern' beyond the passage","Opposite direction — \"concern\" is the opposite of noting findings","Off-topic — outrage is not mentioned at all"], a:1, e:"\"Outraged\" is a stronger, more intense version of \"concern.\" The passage supports concern; it doesn't support outrage. Intensity mismatch = extreme language distortion.", trap_type:"extreme_language" },
      { p:null, q:"A choice includes \"proven\" as in \"The author considers the theory proven.\" The passage says \"evidence supports the theory.\" What is wrong with the choice?", c:["Nothing — \"supports\" and \"proven\" are synonymous","'Evidence supports' is weaker than 'proven' — the choice overstates certainty","The passage never mentions theory, so the choice is completely off-topic","The author's opinion is not relevant to this question type"], a:1, e:"\"Evidence supports\" → likely, probable. \"Proven\" → established as certain fact. The choice escalates epistemic certainty beyond what the passage establishes. This is a strength-of-claim distortion.", trap_type:"extreme_language" },
      { p:null, q:"You are down to two choices: A and C. A says \"The author suggests the policy was flawed.\" C says \"The author implies the policy was deeply misguided.\" The passage says: \"The policy produced mixed results and was later revised.\" Which is supported?", c:["A — problems are noted, but not deep condemnation","C — \"mixed results\" implies deep misgiving","Neither — the passage is purely neutral","Both — they mean the same thing"], a:0, e:"Answer choice language must match passage language. \"Mixed results\" + \"revised\" = problems noted, but not the strong negative intensity of \"deeply misguided.\" C uses stronger language than the evidence supports. Eliminate C.", trap_type:"extreme_language" },
      { p:null, q:"An answer choice says \"The legislation completely solved the problem.\" The passage says \"The legislation addressed several key aspects of the problem.\" What type of elimination trap is this?", c:["A scope error — the choice widens the passage's range","An opposite-direction trap — it reverses the passage's point","A recycled-vocabulary trap — passage words, twisted meaning","Extreme language — \"completely solved\" vs. \"addressed several aspects\""], a:3, e:"\"Completely solved\" ≠ \"addressed several aspects.\" The choice escalates partial progress to complete resolution — an extreme language distortion. Eliminate.", trap_type:"extreme_language" },
      { p:null, q:"Choice B says \"Historians have universally condemned the decision.\" The passage says \"Many historians have questioned the decision.\" What is wrong with Choice B?", c:["Nothing — \"questioned\" implies condemnation","The word \"historians\" is not in the passage","'Universally' adds scope; 'condemned' adds intensity","The choice is in the wrong tense"], a:2, e:"Double distortion: \"universally\" (extreme scope) + \"condemned\" (extreme intensity beyond \"questioned\"). Two elimination triggers in one choice — these are the easiest to eliminate once you know the pattern.", trap_type:"extreme_language" },
      { p:"The results were mixed: some regions improved, others declined, and several showed no measurable change.", q:"Which choice is NOT supported?", c:["Outcomes varied across regions","Some areas saw improvement while others declined","The results did not show uniform improvement","All regions showed some degree of change"], a:3, e:"\"All regions showed some degree of change\" is contradicted by \"several showed no measurable change.\" C is not only unsupported — it's contradicted. The passage explicitly says some regions had no change.", trap_type:"extreme_language" },
      { p:null, q:"You have two choices remaining. Choice A uses precise hedging (\"may suggest,\" \"appears to indicate\"). Choice B makes a definitive claim (\"proves,\" \"demonstrates conclusively\"). The passage is careful and measured in its claims. Which is more likely correct?", c:["A — its certainty level matches the passage's measured tone","B — definitive language matches what questions are testing","Neither — certainty level in choices is irrelevant","B — more specific choices are usually correct"], a:0, e:"Match the certainty level of the passage. A careful passage with hedged language (may, could, suggests) will have a correct answer that also hedges. Choices claiming more certainty than the passage are eliminated.", trap_type:"extreme_language" },
      { p:null, q:"An answer choice says \"The author provides extensive historical evidence.\" The passage provides two historical examples in a 5-paragraph essay focused mainly on present implications. What error does the choice make?", c:["Opposite direction — the passage is anti-historical","Scope error — history vs. present","Off-topic — historical evidence is never mentioned","Exaggeration — 'extensive' overstates two brief examples"], a:3, e:"\"Extensive\" is a quantity exaggeration. Two examples in a modern-focused essay is \"some historical evidence\" — not extensive. Choices that overstate the prominence or quantity of evidence in the passage are eliminable.", trap_type:"extreme_language" },
    ],
    R2:[
      { p:null, q:"The evidence question asks which lines \"best support\" the claim that the author is skeptical. You find three choices that each contain the word \"skeptical\" in the excerpt. What is the risk?", c:["Recycled language: the word may appear without showing skepticism in context","No risk — the word \"skeptical\" directly supports the claim","Choices with the target word are always correct","One of the three is correct, and you should pick the longest"], a:0, e:"Evidence questions can trap you with choices that use the exact word from the claim but in ways that don't actually demonstrate the claim. Look for choices where the passage's language demonstrates the quality — not just names it.", trap_type:"recycled_language" },
    ],
    R3:[
      { p:null, q:"A choice says \"The author disagrees with all critics of the policy.\" The passage says \"The author acknowledges some critics raise valid concerns while disagreeing with others.\" What type of error is this?", c:["The choice correctly identifies the author's position","It erases the nuance: the author partially agrees with critics, not disagrees with all","A scope error — the choice widens the range of critics discussed","An opposite-direction error — it reverses the author's stated view"], a:1, e:"The passage shows a nuanced position: partially agree + partially disagree. The choice collapses this to \"disagrees with all\" — erasing the partial agreement. Nuance erasure is a common recycled-language trap.", trap_type:"half_right" },
      { p:null, q:"In eliminating \"half-right, half-wrong\" choices, what is the recommended test?", c:["Check if the first word of the choice matches the passage","Test every claim in the choice: one unsupported claim eliminates the entire choice","Check if the choice is consistent with your prior knowledge of the topic","Check if the choice mentions the same characters as the passage"], a:1, e:"A choice is only as strong as its weakest link. \"The author admires the policy and believes it was effectively implemented\" — if \"effectively implemented\" isn't supported, the whole choice is wrong, even if \"admires\" is correct.", trap_type:"half_right" },
      ...
      { p:null, q:"You eliminate A and B. Choice C says \"the government was primarily motivated by economic concerns.\" Choice D says \"the government was motivated by economic and political concerns.\" The passage discusses economic pressure but makes no mention of political motivation. Which survives?", c:["D — broader is safer","Both — one concern implies others","Neither — the passage must explicitly state motivation for either to be correct","C — it stays within what the passage actually says"], a:3, e:"Go with the scope the passage establishes. The passage discusses economic pressure — C stays within that scope. D adds \"political concerns\" not mentioned in the passage. Adding information = distortion.", trap_type:"half_right" },
    ],
    R4:[
      ...
      { p:"The explorer described the journey as \"harrowing but ultimately worthwhile.\" Her account emphasizes the physical toll and the dangers faced.", q:"Which choice is supported by the passage?", c:["The explorer regretted undertaking the journey","The journey was more dangerous than any previously attempted","The explorer's physical health was permanently damaged","The explorer found the journey difficult but valuable"], a:3, e:"\"Harrowing but ultimately worthwhile\" directly maps to \"difficult but valuable.\" A contradicts \"worthwhile.\" C introduces comparison information not in the passage. D extends \"physical toll\" beyond what's stated.", trap_type:"could_be_true" },
      { p:null, q:"Which behavior most commonly produces wrong answers on SAT Reading?", c:["Spending too long on the passage before looking at questions","Predicting before reading choices","Answering from outside knowledge instead of the passage","Eliminating obviously wrong choices first"], a:2, e:"The #1 wrong answer generator: using outside knowledge. The SAT passage is the only evidence that counts. If the passage doesn't say it, it's not the answer — regardless of whether it's factually true.", trap_type:null },
      { p:"Research suggests that multitasking, widely considered an asset in modern work environments, may actually reduce productivity and increase error rates.", q:"Based ONLY on this passage, which conclusion is supported?", c:["Research findings challenge the popular positive perception of multitasking","Workers should never multitask under any circumstances","Multitasking is universally harmful in all contexts","Multitasking should be discouraged by employers"], a:0, e:"The passage says multitasking is \"widely considered an asset\" but \"may actually\" reduce productivity. This challenges the common perception. A and D prescribe action not in the passage. B uses \"universally\" — too strong. C is directly supported.", trap_type:"extreme_language" },
      { p:null, q:"An answer choice for a \"main idea\" question reads: \"The author believes renewable energy will eventually replace all fossil fuels.\" The passage argues that renewable energy investment should be expanded. What error does the choice make?", c:["It adds a future prediction the passage never makes","It changes the subject of the passage","It uses the word \"believes\" incorrectly","It contradicts the passage's position on fossil fuels"], a:0, e:"The passage argues for investment expansion. The choice converts an argument for action into a prediction about an inevitable future outcome. That's a different claim — the passage never predicts replacement.", trap_type:"could_be_true" },
      { p:null, q:"A question asks for \"textual evidence that best supports the conclusion in the previous question.\" You answered the previous question correctly: \"The author doubts the effectiveness of the new regulation.\" Which excerpt best supports this?", c:["'The regulation passed with bipartisan support.'","'Critics have long argued for stricter oversight.'","'The regulation was first proposed in 2019.'","'Whether these measures will achieve their goals remains to be seen.'"], a:3, e:"\"Whether such measures will achieve their intended goals remains to be seen\" directly expresses doubt about effectiveness. A shows support for the regulation. B is about critics (not the author). D is a factual date.", trap_type:"true_but_irrelevant" },
      { p:"The expedition team returned with extensive data. \"Our findings,\" wrote the lead researcher, \"suggest we may have fundamentally misunderstood the ecosystem's resilience.\"", q:"Which choice provides the best evidence that the expedition produced surprising results?", c:["The team returned with extensive data","The lead researcher wrote about the findings","The statement suggests prior assumptions were challenged","The expedition team completed their mission"], a:2, e:"\"May have fundamentally misunderstood\" = previous assumptions challenged = surprised by results. A is about data quantity, not surprise. B is about who wrote, not what was found. D is neutral completion.", trap_type:"true_but_irrelevant" },
      { p:"A geologist studying sediment cores noted regular alternating layers of light and dark material. The dark layers corresponded to periods of high biological productivity; the light layers, to periods of reduced activity. Over 40,000 years, the pattern repeated with remarkable consistency.", q:"What does the 40,000-year regularity most strongly suggest?", c:["The sediment cores are unreliable data sources","A cyclical external force, not random variation, drove the pattern","Biological productivity is impossible to measure over long time periods","The geologist made a calculation error"], a:1, e:"Regularity over a long consistent cycle implies a driving external mechanism — likely orbital or climatic cycles. Random variation does not produce regular repeating patterns. This is an inference from the data pattern described.", trap_type:"could_be_true" },
      { p:null, q:"A 500-word passage describes how monarch butterflies navigate using the sun. A question asks: 'What can be inferred about monarch navigation on overcast days?' The passage does not mention overcast days. Your inference must be based on:", c:["The passage's logic — sun navigation implies disruption when overcast","What you know about butterflies from biology class","Any reference to weather in the passage","A guess — you cannot infer what the passage does not say"], a:0, e:"Inference questions test what logically follows from the passage's stated logic. If the passage says monarchs navigate by sun, it logically follows that removing the sun disrupts navigation. Valid inferences extend the passage's logic, not outside knowledge.", trap_type:"could_be_true" },
      { p:"The city's growth accelerated after the railroad arrived in 1887. Population tripled within a decade. By 1900 it had overtaken its rival to the east. Yet the prosperity was uneven — longshoremen and factory workers saw wages stagnate even as merchants and landowners accumulated significant wealth.", q:"What does the passage suggest about the railroad's impact on the city?", c:["The railroad caused the city to grow uniformly and benefit all residents equally","The railroad had no impact on wages","The railroad arrived too late to significantly affect the city's development","Dramatic growth overall, but benefits concentrated at the top"], a:3, e:"Growth evidence: population tripled, overtook rival. But 'prosperity was uneven' with workers' wages stagnating while merchants grew wealthy. The railroad drove growth, but benefits were unequal. B captures both sides.", trap_type:"half_right" },
      ...
      { p:null, q:"A question asks you to 'choose the best evidence for the claim that the author views urban sprawl negatively.' You find: (A) a statistical claim about sprawl's costs, and (B) the author's explicit judgment: 'urban sprawl represents one of the most misguided developments in modern city planning.' Which is better evidence?", c:["The statistical claim — data is always stronger evidence","The explicit judgment — direct evidence of the author's view","Both are equally valid","Neither — evidence for an author's view must come from the conclusion"], a:1, e:"For evidence-of-author's-view questions, explicit authorial evaluation is the strongest evidence. Statistics describe a topic; judgment words like 'misguided' directly express the author's position.", trap_type:"extreme_language" },
    ],
    R5:[
      { p:null, q:"Which pair of answer choices demonstrates the \"opposite direction\" trap?", c:["\"The author criticizes the policy\" vs. \"The author endorses the policy\"","\"The author supports the policy\" vs. \"The author advocates for the policy\"","\"The author analyzes the data\" vs. \"The author examines the data\"","\"The author notes limitations\" vs. \"The author acknowledges constraints\""], a:0, e:"\"Criticizes\" and \"endorses\" are antonyms — one positive, one negative. If the passage clearly shows one direction, the opposite direction choice is an elimination candidate. Pairs A, C, D are near-synonyms, not opposites.", trap_type:"opposite_trap" },
      { p:"The researcher concluded that the drug reduced symptoms in 60% of patients. The remaining 40% saw no significant change.", q:"Which choice accurately represents the passage?", c:["The drug cured all patients who took it","The drug was largely ineffective across the patient population","The drug produced identical results in all patients","The drug was effective for a majority of patients but not for all"], a:3, e:"60% = majority; 40% saw no change = not effective for all. B is precisely accurate. A (cured all) and D (identical results) contradict the split; C misrepresents 60% effective as \"largely ineffective.\"", trap_type:"opposite_trap" },
      { p:null, q:"An answer choice says: \"The author implies critics are completely wrong.\" The passage says: \"While the author challenges certain criticisms, she acknowledges the core concern has merit.\" What error does the choice make?", c:["A scope error — the range of critics is widened","Extreme language in the word 'implies'","It reverses the passage — the author grants the concern merit","Off-topic — critics are not the main subject"], a:2, e:"The passage shows the author partially defending critics (core concern has merit). \"Completely wrong\" is both extreme and opposite to the partial agreement. Two errors: intensity + direction.", trap_type:"opposite_trap" },
    ],
    R6:[
      ...
      { p:null, q:"An answer choice accurately describes something discussed in paragraph 3. But the question asks for the main idea of the PASSAGE. What do you do with this choice?", c:["Select it — if it's accurate, it's the answer","Partially credit it — mark it as a backup choice","Check if it appears in any other paragraph","Eliminate it — accurate detail ≠ main idea"], a:3, e:"Accurate ≠ main idea. A passage's main idea encompasses the whole text. A choice that only captures one section's content is a supporting-detail trap, not the main purpose.", trap_type:"true_but_irrelevant" },
    ],
    R7:[
      ...
      { p:null, q:"A claim question asks: \"The author suggests traditional methods are inadequate.\" You answer: \"The author believes new methods are superior.\" Are these the same?", c:["Yes — if new is superior, traditional must be inadequate","Yes — they are logically equivalent","No — but you should pick evidence that supports both simultaneously","No — they are different claims; evidence for one doesn't support the other"], a:3, e:"These are different logical claims. A passage could argue new methods are better without saying traditional methods are inadequate (they may have been appropriate for their time). Don't confuse \"A is better\" with \"B is bad.\"", trap_type:"comparison_fallacy" },
    ],
    R8:[
      ...
      { p:null, q:"A choice says: \"The author argues that climate policy has failed globally.\" The passage says: \"Climate policy has seen mixed results in several European nations.\" Which error does the choice make?", c:["It stretches 'several nations' into 'globally'","It uses vocabulary not in the passage","It incorrectly identifies the author's tone","It changes past tense to present tense"], a:0, e:"Scope error: \"several European nations\" ≠ \"globally.\" This is a classic elimination trigger. If the choice expands geographic, temporal, or categorical scope beyond the passage, eliminate it.", trap_type:"scope_trap" },
      { p:null, q:"Answer choice D says \"The policy was primarily motivated by economic concerns.\" The passage discusses political pressure, public health, and economic considerations — all mentioned. What makes D eliminable?", c:["Nothing — economic concerns are mentioned in the passage","Economic concerns make D a scope distortion","The choice uses vocabulary not in the passage","'Primarily' imposes a ranking the passage never makes"], a:3, e:"\"Primarily\" is a ranking word — it says economic concerns were #1. If the passage lists three equal factors, no choice that claims primacy of one factor is supported.", trap_type:"scope_trap" },
    ],


    // ── BATCH 2: Reading Passage (R9–R13) + Grammar (G1–G7) ──
    R9:[
      { p:null, q:"You read a passage about a scientist's claim and the evidence challenging it. An inference question asks: 'What would the scientist most likely say in response to the new evidence?' Your answer should be based on:", c:["What a scientist would typically say in such a situation based on general knowledge","The new evidence alone","The passage's conclusion about who is correct","The scientist's stated reasoning in the passage, extended to the new evidence"], a:3, e:"You are asked to extend the scientist's argument — not invent a response. Use the scientist's actual stated reasoning as your foundation. What would that specific argument say about the specific new evidence?", trap_type:"could_be_true" },
    ],
    R10:[
    ],
    R11:[
      ...
      { p:null, q:"A question asks: \"Which choice best describes the overall structure of the passage?\" The passage starts by presenting a problem, then provides historical context, then argues for a specific solution. The best structural description is:", c:["Problem → historical context → solution advocated","Comparison and contrast of two solutions","Chronological history of a topic","Argument → counterargument → rebuttal"], a:0, e:"Match the actual structure you observed. This passage has three distinct moves: problem, history, solution. Only B captures all three in order.", trap_type:null },
      { p:null, q:"Why is identifying passage structure useful before reading?", c:["It lets you skip paragraphs you've identified as unimportant","It tells you exactly where to find the main idea (always in paragraph 1)","Structure identification always reveals the answer to main purpose questions","It creates a mental map for faster comprehension and prediction"], a:3, e:"A structural map (intro → context → argument → conclusion) tells you which paragraph to look in when a question asks about a specific function. It's orientation, not a shortcut.", trap_type:null },
      { p:null, q:"A passage has this structure: P1 introduces Claim A, P2 introduces Claim B, P3 compares them, P4 concludes which is superior. A question asks: \"In which paragraph does the author most directly state a preference?\" You should look in:", c:["P1 — first claims are usually strongest","P2 — second arguments are often preferred","P3 — comparison reveals preference implicitly","P4 — preference is stated most directly in the conclusion"], a:3, e:"In evaluation/comparison structures, the conclusion (P4) is where the author states which is better. Implicit preference in P3 ≠ \"most directly stated.\" P4 is the right paragraph.", trap_type:null },
      { p:null, q:"You're mapping a passage and find P2 is a counterargument. A question asks: \"The author introduces the information in paragraph 2 in order to ___.\" The answer most likely is:", c:["Present a view the author will then challenge or qualify","Provide supporting evidence for the main argument","Summarize the passage's main point","Introduce new evidence the author endorses"], a:0, e:"Counterargument paragraphs exist to be addressed/rebutted. The function of P2 (counterargument) is: \"present a view that the author will then challenge.\" This is standard argumentative structure.", trap_type:"opposite_trap" },
      { p:null, q:"A passage begins: \"Despite widespread assumptions that X causes Y, recent studies suggest the relationship is more complex.\" What structural role does this opening sentence play?", c:["It sets the central tension: common assumptions vs. new research","It introduces supporting evidence for X causing Y","It provides a definition of terms","It is a transitional statement with no structural significance"], a:0, e:"\"Despite widespread assumptions... recent studies suggest\" is a classic contrast structure. The passage will challenge a conventional belief with new evidence. This sentence announces the central tension.", trap_type:"opposite_trap" },
      { p:null, q:"You are told a passage discusses two contrasting theories. After reading, a question asks: \"The author's own position is best described as ___.\" Where should you look first?", c:["Whichever theory is introduced first — that's always the author's preferred one","The middle of the passage — balanced positions appear there","The passage title alone","The conclusion — authors typically signal preference there"], a:3, e:"In a passage comparing two theories, the author's preference is most clearly signaled in the conclusion or thesis statement. Middle paragraphs are usually dedicated to presenting both sides fairly.", trap_type:"extreme_language" },
      { p:"In her essay, Chen argues that contemporary urban design fails to account for how people actually experience cities. She draws on neurological research demonstrating that irregular, complex street patterns activate more of the brain's navigational and aesthetic processing than grid layouts. 'A city,' she writes, 'should be experienced as a living organism, not a spreadsheet.'", q:"Chen's comparison of a city to a 'living organism' rather than a 'spreadsheet' primarily serves to", c:["Provide scientific evidence for her claim","Introduce the topic of urban planning history","Contrast sensory richness with the cold efficiency she critiques","Argue that spreadsheets are an inadequate planning tool"], a:2, e:"Organism = living, complex, sensory. Spreadsheet = cold, grid-based, efficiency-focused. The contrast in the metaphor does rhetorical work: it embeds her critique of overly mechanistic urban planning in the language itself.", trap_type:"true_but_irrelevant" },
      { p:null, q:"An author uses a lengthy anecdote about one traveler's experience to introduce an argument about the unreliability of eyewitness testimony. What function does the anecdote serve?", c:["It provides statistical evidence for the argument","It makes the abstract argument concrete and accessible","It contradicts the author's main claim","It is the author's personal experience"], a:1, e:"Opening anecdotes: (1) engage the reader, (2) make abstract arguments accessible, (3) provide a specific case the author can reference. They are rhetorical tools, not statistical evidence.", trap_type:"true_but_irrelevant" },
      { p:"The author writes: 'One could argue, of course, that these incentives are sufficient. One would be wrong.'", q:"The author's use of 'One could argue... One would be wrong' is best understood as", c:["An acknowledge-then-reject move, emphatic and dismissive","A respectful acknowledgment of the opposing view","A neutral presentation of both sides","A concession that weakens the author's position"], a:0, e:"The construction acknowledges then immediately refutes. 'One would be wrong' is emphatic rejection, not a concession. This rhetorical device takes the opposing view seriously enough to mention it, then flatly dismisses it.", trap_type:"opposite_trap" },
      { p:null, q:"An author of a science passage uses a simile comparing a cell's membrane to a nightclub bouncer. What is the primary rhetorical purpose?", c:["To provide a precise scientific definition of membrane function","To argue that cellular biology is entertaining","To introduce membrane biology historically","To make selective permeability understandable via a familiar comparison"], a:3, e:"Similes in explanatory writing bridge specialist knowledge and general understanding. 'Bouncer = selective admittance' makes selective membrane permeability immediately intuitive. Purpose: clarification for non-specialist readers.", trap_type:"true_but_irrelevant" },
      { p:"Critics of the proposal were numerous and vocal. Yet the committee moved forward. In retrospect, this was perhaps the defining error of the administration.", q:"The phrase 'in retrospect' signals that the author is", c:["Expressing uncertainty about whether an error occurred","Quoting another source's assessment","Evaluating the decision from a later vantage point","Predicting a future outcome"], a:2, e:"'In retrospect' = backward-looking evaluation. The author is describing how the error became clear after the fact — with the benefit of knowing what followed. It signals post-hoc judgment.", trap_type:"opposite_trap" },
    ],
    R12:[
      ...
      { p:null, q:"A passage presents a scientist's claim followed by three paragraphs of counter-evidence. You are asked what the passage is \"primarily concerned with.\" Which framing is correct?", c:["The specific counter-evidence in paragraph 3","The scientist's original experiment design","Whether a scientific claim holds up against challenges","The history of the research field"], a:2, e:"PCE: \"primarily concerned with\" = main purpose. The passage structure (claim → 3 counter-paragraphs) is an evaluation structure. A narrows to one paragraph; C and D are not in the text.", trap_type:"true_but_irrelevant" },
      { p:"The committee voted unanimously. The result surprised no one who had attended the earlier closed-door sessions.", q:"The most complete summary of this two-sentence passage is:", c:["A committee made a unanimous vote","Closed-door sessions always predict public votes","Committees are predictable organizations","The vote was unsurprising given earlier private decisions"], a:3, e:"PCE: Sentence 1 = the event; sentence 2 = context that makes it unsurprising. The complete summary captures both. A ignores sentence 2. C and D are overgeneralizations not in the text.", trap_type:"half_right" },
      { p:null, q:"You're asked: \"The author's main purpose is to ___.\" You find three competing answers that all reference content actually in the passage. How do you distinguish the main purpose from a supporting detail?", c:["Pick the answer that the whole passage serves","Pick the answer that covers the most word count","Pick the answer that appears in the first paragraph","Pick the most specific answer"], a:0, e:"Main purpose = what the whole passage does. Supporting details = what individual sections do. If an answer only describes one section, eliminate it. Coverage of the entire passage is the test.", trap_type:null },
      { p:"The urban heat island effect raises city temperatures by 2–5°F. This affects energy consumption, public health, and infrastructure longevity. Mitigation strategies include green roofs, increased tree cover, and reflective paving.", q:"The primary purpose of this passage is to", c:["Advocate for green roofs as the best mitigation","Criticize urban planners for causing the heat island effect","Provide historical background on city temperature measurement","Describe a phenomenon, its effects, and potential solutions"], a:3, e:"PCE: The passage does three things — names the effect, lists its consequences, offers solutions. B captures all three. A narrows to one solution; C adds a critical tone not in the text; D is not present.", trap_type:"half_right" },
      { p:null, q:"The question asks for the author's \"central claim.\" You find two choices: (A) \"Renewable energy investment has increased since 2010,\" and (B) \"Renewable energy is the most viable long-term solution to climate change.\" The passage discusses both trends since 2010 AND argues for renewables long-term. Which is the central claim?", c:["A — trends are more specific and verifiable","Neither — both are equally central","B — the passage argues for it; trends are evidence","A — it appears earlier in the passage"], a:2, e:"Central claim = the argument the passage makes. Trends (A) are evidence supporting the argument (B). Evidence ≠ claim. B is what the passage is trying to convince the reader of.", trap_type:null },
      { p:"Scientists have long studied how sleep affects memory consolidation. Recent research suggests that during REM sleep, the brain actively replays and encodes experiences from the day. This process appears to be critical for long-term retention.", q:"The main idea of this passage is best expressed as:", c:["REM sleep plays a key role in processing and retaining memories","REM sleep is one of several stages in the sleep cycle","Scientists have studied sleep for a long time","Recent research methods are more advanced than older ones"], a:0, e:"The passage covers: sleep → memory consolidation → REM specifically → long-term retention. B captures the relationship between sleep (specifically REM) and memory. A narrows to structure; C is background; D is not stated.", trap_type:"true_but_irrelevant" },
      { p:null, q:"You are choosing between: (A) \"The passage argues that economic inequality worsened during the recession.\" (B) \"The passage argues that economic inequality, structural vulnerability, and policy failures all contributed to the prolonged recession.\" The passage has sections on all three topics. Which is the main idea?", c:["B — it captures all three factors; A captures only one","A — inequality is discussed first and most extensively","A — simpler choices are usually correct for main idea","B — longer choices are usually main ideas"], a:0, e:"If the passage argues for multiple contributing factors, the main idea must reflect all of them. A captures only one factor — it's accurate but incomplete. Incomplete = wrong for \"main idea\" questions.", trap_type:"half_right" },
      { p:null, q:"The difference between a passage's \"main idea\" and its \"primary purpose\" is:", c:["They are the same — both describe the overall argument","Primary purpose is always stated in the first sentence; main idea is in the last","Main idea applies to fiction; primary purpose applies to non-fiction","Main idea = what the author argues; purpose = why they wrote it"], a:3, e:"Main idea: \"The author argues that X causes Y.\" Primary purpose: \"To challenge conventional assumptions about Z.\" One is the content, the other is the function/goal of the writing.", trap_type:null },
      { p:null, q:"A passage has a two-sentence intro, six body paragraphs of evidence, and a one-paragraph conclusion. The main idea question asks you to pick the best summary. You should look primarily at:", c:["The intro — main ideas are always introduced first","The conclusion — main ideas are always restated there","The intro and conclusion together — they bracket the claim","The longest body paragraph — it contains the most evidence"], a:2, e:"Intro + conclusion together usually bracket the main idea. The intro introduces it; the conclusion may sharpen or synthesize it. Neither alone is always sufficient — use both.", trap_type:"extreme_language" },
    ],
    R13:[
    ...
    G3:[
      { p:null, q:"You need to choose between 'Furthermore' and 'However' as a transition. Sentence 1: 'The treatment reduced pain.' Sentence 2: '___, it produced significant side effects.' Which is correct?", c:["However — it introduces a contrast","Furthermore — adding another quality","Therefore — showing causation","Additionally — adding a related point"], a:0, e:"'However' introduces contrast or complication. S1 is positive (reduced pain); S2 is negative (side effects). Contrast relationship = 'However.'", trap_type:null },
      { p:null, q:"A question asks which sentence 'most logically concludes the paragraph.' The paragraph describes three pieces of evidence for climate change. The most logical conclusion is:", c:["A sentence introducing a new, unrelated topic","A sentence about the history of climate science","A sentence offering a counterargument to climate change","A sentence synthesizing what the evidence collectively shows"], a:3, e:"A concluding sentence synthesizes evidence and draws a conclusion — it does not introduce new topics, provide irrelevant history, or contradict the paragraph's established point.", trap_type:null },
      { p:null, q:"The best transition between a paragraph describing a problem and a paragraph proposing a solution is:", c:["'In sharp contrast, ...'","', and therefore, ...'","'To address this challenge, ...'","'In a similar way, ...'"], a:2, e:"'To address this challenge' explicitly links the problem (just described) to the solution (about to be proposed). It signals the pivot from problem identification to problem-solving. Other choices signal contrast, addition, or similarity — not problem-to-solution.", trap_type:null },
    ],
    G4:[
      { p:null, q:"A sentence reads: 'The report ___ was released last week contained several errors.' No comma precedes the blank. Should you use 'which' or 'that'?", c:["'that' — restrictive clauses take 'that,' no comma","'which' — both words are interchangeable","'which' with a comma always","'who' — used for important documents"], a:0, e:"Restrictive clauses (essential to identify what is being discussed) use 'that' without a comma. Non-restrictive clauses (added, removable information) use 'which' with a comma. No comma + essential identification = 'that.'", trap_type:null },
      { p:null, q:"A sentence uses 'however' between two independent clauses with only a comma: 'The results were promising, however the sample size was small.' What is the error?", c:["No error — the sentence is correct as written","The sentence needs 'but' instead of 'however'","The comma should be removed","Comma splice: a semicolon is needed before 'however'"], a:3, e:"'However' is not in FANBOYS. Two independent clauses joined only by a comma = comma splice. Correct: semicolon before 'however' + comma after. Or: split into two sentences.", trap_type:null },
      { p:null, q:"A sentence reads: 'The company was founded in 1985; it grew rapidly.' Is the semicolon used correctly?", c:["No — semicolons can only separate items in a list","No — a semicolon requires a transition word after it","Yes — a semicolon joins two closely related independent clauses","No — a comma should be used instead"], a:2, e:"Semicolons join two independent clauses without a conjunction when the ideas are closely related. Both 'The company was founded in 1985' and 'it grew rapidly' are independent clauses. Correct.", trap_type:null },
      { p:null, q:"A sentence reads: 'The results were clear, however, further testing was recommended.' What is the error?", c:["No error — the punctuation is correct as written","Comma splice — a semicolon is required before 'however'","The second comma after 'however' should be removed","A colon should replace the first comma"], a:1, e:"Comma + 'however' + comma between two independent clauses = comma splice. 'However' is a conjunctive adverb, not a coordinating conjunction. Correct: semicolon before 'however' + comma after.", trap_type:null },
      { p:null, q:"When should a colon be used in a sentence?", c:["To introduce a list or explanation after an independent clause","After any introductory phrase in a sentence","Between the subject and the predicate of a clause","Before any coordinating conjunction like 'and'"], a:0, e:"Colon rule: the material before the colon must be an independent clause. 'The experiment had three stages: design, execution, and analysis' ✓. 'The stages were: ...' ✗ — 'were' requires a direct object, not a colon.", trap_type:null },
      { p:null, q:"The sentence reads: 'She bought: apples, oranges, and grapes.' Is the colon used correctly?", c:["Yes — the colon introduces a list","Yes — colons always introduce lists","No — a semicolon should be used instead","No — a colon can't interrupt a verb and its objects"], a:3, e:"A colon cannot interrupt a verb-object relationship. 'She bought [objects]' — the objects are direct objects, not a separately introduced list. Remove the colon entirely.", trap_type:null },
      { p:null, q:"Which sentence uses punctuation correctly?", c:["The three cities visited were; Paris, Rome, and Tokyo.","The three cities visited were, Paris, Rome, and Tokyo.","The three cities visited were: Paris, Rome, and Tokyo.","The project required three skills patience persistence and creativity."], a:2, e:"B is correct: independent clause + colon + list. A uses a semicolon incorrectly after 'were.' C uses a comma incorrectly. D lacks any punctuation before the list (needs a colon).", trap_type:null },
    ],
    G5:[
      ...
      { p:null, q:"The sentence reads: 'The group of students were studying late.' Should the verb be 'was' or 'were'?", c:["'were' — 'students' is closest to the verb and is plural","Either is acceptable","The sentence should be rewritten","'was' — the subject is the singular noun 'group'"], a:3, e:"Subject-verb agreement: the subject is 'group' (singular), not 'students' (prepositional phrase object). Do not be distracted by the noun closest to the verb — find the actual grammatical subject.", trap_type:null },
      { p:null, q:"A sentence reads: 'Neither the manager nor the employees was aware of the change.' Is 'was' correct?", c:["Yes — 'neither...nor' always takes singular","Yes — 'employees' is a collective noun here","No — the verb agrees with the nearer subject: 'were'","No — 'neither...nor' always takes plural"], a:2, e:"'Neither...nor' rule: verb agrees with the CLOSER subject. 'Employees' (plural) is closer to the verb → 'were.' If it were 'employees or the manager,' the verb would be singular. Proximity determines agreement.", trap_type:null },
      { p:null, q:"The subject is 'the quality of the recommendations.' The verb should agree with:", c:["'quality' (singular) → 'is'","'recommendations' (plural) → 'are'","Whichever noun is most important","The entire noun phrase, treated as collective"], a:0, e:"Subject-verb agreement: identify the actual subject, not the object of a prepositional phrase. 'Quality' is the subject; 'of the recommendations' is a prepositional phrase modifier. 'Quality is' — singular.", trap_type:null },
      { p:null, q:"The sentence reads: 'Five dollars are a fair price.' What is the error?", c:["Money as a single sum takes a singular verb: 'Five dollars is'","No error — 'dollars' is plural, so 'are' is correct","The sentence needs 'seem' instead of 'are'","No error — it depends on whether dollars refers to bills or an amount"], a:0, e:"When a quantity of money, time, or distance is treated as a single unit, use singular: 'Five dollars IS a fair price.' Compare: 'Five dollar bills are scattered on the table' (individual items = plural).", trap_type:null },
      { p:null, q:"The sentence reads: 'Politics are a contentious subject.' Is 'are' correct?", c:["Yes — 'politics' ends in -s so it is plural","Yes — all -ics words are plural","No — 'politics' always takes 'are' regardless of context","No — fields of study like 'politics' take singular verbs"], a:3, e:"Fields of study ending in -ics (politics, economics, mathematics, physics, ethics) are treated as singular nouns. 'Mathematics is difficult.' 'Politics is contentious.' Distinct from: 'Her politics are moderate' (plural noun).", trap_type:null },
      { p:null, q:"The sentence reads: 'Everyone on both teams have signed the waiver.' What is the correction?", c:["'Everyone... have' is correct — 'teams' is plural","'All of everyone... has'","'has' — 'everyone' is always singular","No correction needed"], a:2, e:"'Everyone,' 'someone,' 'anyone,' 'no one,' 'everybody' are indefinite pronouns and always singular. 'Everyone has signed' — regardless of 'on both teams.' The prepositional phrase modifies everyone, does not change its number.", trap_type:null },
      { p:null, q:"The sentence reads: 'The researcher, along with his colleagues, are presenting at the conference.' What is the error?", c:["No error — 'colleagues' is plural, so 'are' is correct","A verb-tense error in 'are presenting'","The subject is 'researcher' — the verb should be 'is'","A pronoun-agreement error with 'his'"], a:2, e:"'Along with,' 'as well as,' 'together with' are parenthetical phrases — they do not change the grammatical subject. 'Researcher' is singular → 'is presenting.' Do not let parenthetical phrases change your verb.", trap_type:null },
      { p:null, q:"A sentence reads: 'A number of problems have emerged.' Is 'have' correct?", c:["Yes — 'a number of' works like 'several': plural verb","No — 'a number' is singular, so 'has' is correct","Yes — always use plural after 'of'","No — always use singular after 'a number'"], a:0, e:"'A number of' = several → plural verb. 'The number of' = the total count → singular verb. 'A number of problems have emerged' ✓ and 'The number of problems has grown' ✓ — both correct.", trap_type:null },
      { p:null, q:"The sentence reads: 'The team were arguing amongst themselves.' In American English standard, is this correct?", c:["Yes — 'were' is always correct for groups","Yes — 'themselves' confirms the plural is intended","No — the sentence should use 'had been arguing'","No — AmE treats collective nouns as singular: 'was arguing'"], a:3, e:"AmE: collective nouns (team, committee, jury) take singular verbs. 'The team was arguing.' BrE may use plural when members act individually. SAT tests AmE conventions.", trap_type:null },
      { p:null, q:"The sentence reads: 'Each of the team members are responsible for their own equipment.' Identify the error.", c:["No error — 'team members' is plural","A pronoun error only, in 'their'","'Each' is singular — the verb should be 'is'","No error if the team is large"], a:2, e:"'Each' is singular → 'is.' 'Each of the team members IS responsible for their own equipment.' (Singular 'they/their' for 'each' is accepted in current usage.)", trap_type:null },
      { p:null, q:"The sentence reads: 'Twenty years have passed since the founding.' Is 'have' correct?", c:["No — 'twenty' requires a singular verb","Yes — plural numbers always take plural verbs","No — the correct verb here is 'is'","Both work — single span 'has,' individual years 'have'"], a:3, e:"'Twenty years' can be treated as a single span (singular: 'has passed') or as individual years (plural: 'have passed'). Both are defensible. SAT questions with this ambiguity usually provide context that clarifies which is intended.", trap_type:null },
      { p:null, q:"A sentence reads: 'Neither the report nor the data support the conclusion.' Is 'support' correct?", c:["Yes — the verb agrees with the nearer noun, 'data' (plural)","No — 'neither...nor' always takes singular, so 'supports' is required","No — 'neither...nor' makes the subject collective, requiring 'supports'","Yes — 'neither...nor' always takes plural"], a:0, e:"'Neither...nor' verb agreement: agree with the closer noun. 'Data' (closer) — if treated as plural (scientific writing), 'support' is correct. If singular, 'supports.' Context and passage register determine which.", trap_type:null },
    ],
    G6:[
      ...
      { p:null, q:"A sentence reads: 'The company increased their profits significantly last quarter.' In formal writing, what is the issue?", c:["No issue — 'company' can take 'their'","The issue is the verb tense of 'increased'","The issue is 'significantly' placement","Formal writing: 'company' is singular, so 'its profits'"], a:3, e:"'Company' is a singular entity → 'its profits.' SAT formal writing requires singular pronoun-antecedent agreement for collective nouns. Match the pronoun to the noun's grammatical number.", trap_type:null },
      { p:null, q:"A sentence reads: 'When a student finishes their exam, they should turn it in immediately.' Is this acceptable?", c:["Yes — singular 'they' is accepted, including on the SAT","No — 'their' and 'they' are plural; 'a student' is singular","No — the sentence must say 'he or she'","Yes — but only when the gender of the student is unknown"], a:0, e:"The SAT now accepts singular 'they/their' as a gender-neutral singular pronoun. 'When a student finishes their exam' — acceptable. Both 'he or she' and 'they' are valid in current standard usage.", trap_type:null },
      { p:null, q:"The sentence reads: 'Each student must bring their own calculator.' Is 'their' correct?", c:["No — 'each' is singular, requiring 'his or her calculator'","No — 'each' makes the noun collective, requiring 'its'","Yes — singular 'their' is standard for 'each student'","Yes — 'their' is always correct after 'each'"], a:2, e:"Both 'his or her' and 'their' are acceptable for singular gender-neutral antecedents. The SAT accepts singular 'they.' If both appear as choices, the question likely has a different grammatical error to identify.", trap_type:null },
      { p:null, q:"A sentence reads: 'The band announced they would go on tour.' Is 'they' correct?", c:["Yes — for bands and teams, both 'it' and 'they' are defensible","No — 'band' is a singular collective noun, requiring 'it would go on tour'","No — 'they' makes the sentence ambiguous","Yes — collective nouns always take plural pronouns"], a:0, e:"Musical groups and sports teams (the band, the team, the squad) are commonly referred to with both 'it' (formal) and 'they' (informal/common). The SAT will clarify through context; if the passage uses a formal register throughout, 'it' may be preferred.", trap_type:null },
    ],
    G7:[
    ],
    ...
    M1:[
      { p:null, q:"You need to solve: 3x + 7 = 22. Choices are: 3, 4, 5, 6. Using Backsolving, which choice do you test first, and what is the result?", c:["Test 3 first: 3(3)+7=16 — too small; correct answer is 5","Test 6 first (largest): 3(6)+7=25 — too large, so 5 is correct","Test the middle value (5 or 4). Test 5: 3(5)+7=22 ✓ — correct answer is 5","Test 3: 3(3)+7=16. Too small. The answer must be 6."], a:2, e:"Backsolve: start with the middle choice. 5 is the middle of {3,4,5,6}. Test: 3(5)+7=22 ✓. Done in one step. If it were too small, test higher; if too large, test lower.", trap_type:null },
      { p:null, q:"You are backsolving and the middle choice gives a value that is too large. What do you test next?", c:["Test the largest choice — it might overshoot and confirm the pattern","Test the smallest choice immediately — skip the second-middle","Re-read the problem; the middle is always correct when backsolving","Test the smaller choices — since the middle was too large, the answer is smaller"], a:3, e:"Backsolving direction: too large → go smaller. Test the next smaller choice. This is always more efficient than testing from the extreme.", trap_type:null },
      { p:null, q:"For which type of problem is Backsolving MOST efficient compared to algebra?", c:["Simple one-step equations: 2x = 10","Multi-step equations with integer choices, e.g., x² + x = 12","Equations where choices are expressions with variables","Systems of two equations with two unknowns"], a:1, e:"Backsolving shines on problems where algebraic manipulation is error-prone or slow, but the choices are specific integers that can be tested quickly. Simple one-step algebra is faster to solve directly.", trap_type:null },
      ...
      { p:null, q:"When backsolving with fractional answer choices, what adjustment should you make?", c:["Avoid backsolving with fractions — use algebra instead","Always test the smallest fraction first","The process is the same — test middle, adjust direction","Fractions make backsolving impossible"], a:2, e:"The Backsolving process is identical for fractions — test middle, adjust direction. The arithmetic is more error-prone, so work carefully, but the strategy is the same. Use Backsolving when it is faster than algebraic manipulation.", trap_type:null },
      { p:null, q:"Which SAT strategy applies when the question asks you to evaluate an answer choice by plugging it back into the original equation?", c:["Plug In Numbers — substituting variable values","Ballpark — estimating to eliminate unreasonable choices","Unit Tracking — following units through calculations","Backsolving — testing each answer choice in the original equation"], a:3, e:"Backsolving = testing answer choices by substituting them into the equation. Plug In Numbers uses invented values to test variable expressions. They're complementary: Backsolving when answers are specific numbers; Plug In Numbers when answers contain variables.", trap_type:null },
    ],
    M2:[
      { p:null, q:"The choices are: (A) n+4, (B) 2n+2, (C) 3n-1, (D) n². You plug in n=4. Your target answer is 10. What do you conclude?", c:["B gives 2(4)+2 = 10 ✓ — likely correct, but verify with a second value","Choice B immediately — any choice that works is correct","All choices that don't equal 10 are permanently eliminated — B is confirmed","Choice D: 16 — eliminate. Answer must be B or C."], a:0, e:"At n=4: A=8, B=10✓, C=11, D=16. B works. But at one value, multiple choices could match — always verify with a second value (n=3: B=8; target should be 8 if B is correct). Confirm before selecting.", trap_type:null },
      ...
      { p:null, q:"Why avoid 0 and 1 when using Plug In Numbers?", c:["They are forbidden by SAT calculator-section rules","Special properties (n×0=0, n¹=n) make wrong choices match the right one","They make the arithmetic harder than it needs to be","They only work on odd-numbered problems in each module"], a:1, e:"0 neutralizes multiplication (n×0=0 for everything). 1 trivializes powers and multiplication. Both can make wrong choices appear correct. Use 2, 3, or 4 instead — specific enough to differentiate choices.", trap_type:null },
      { p:null, q:"A problem asks: 'Which expression is equivalent to (2x+3)² ?' You plug in x=2. Your target: (2(2)+3)²=49. You evaluate choices: A=4x²+9, B=4x²+12x+9, C=4x²+6x+9, D=2x²+12x+9. At x=2: A=25, B=49✓, C=49✓. What do you do?", c:["Select B — it appeared first","Select C — it appeared last","Try a second value to separate B and C","Both B and C are correct"], a:2, e:"When two choices both work at the first test value, try a second. At x=1: B=4+12+9=25, C=4+6+9=19, target=(2+3)²=25. B works at x=1; C does not. B is correct. (Algebraic check: (2x+3)²=4x²+12x+9 ✓)", trap_type:null },
      { p:null, q:"Plug In Numbers works best when:", c:["The answer choices contain specific numbers","The choices are variable expressions you can test with numbers","The problem contains only one variable in total","The problem involves geometry diagrams only"], a:1, e:"Plug In Numbers is designed for variable-choice problems where algebra would be abstract and error-prone. Plugging in a specific number converts the abstract into concrete arithmetic you can verify.", trap_type:null },
    ],
    M3:[
      { p:null, q:"A price increased by 20%, then decreased by 20%. Using 100 as the starting value, what is the final price?", c:["96 — the second percentage applies to a different base","100 — the increase and decrease cancel out exactly","80 — the two changes produce a straight net decrease","104 — the changes produce a slight net increase"], a:0, e:"Start: 100. Up 20%: 100×1.20=120. Down 20%: 120×0.80=96. Net: 96, not 100. Percentages compound on changing bases. Up then down by the same percent always yields a net decrease.", trap_type:null },
      { p:null, q:"A store marks up an item by 50%, then offers a 25% discount. Using 100 as the base, what is the final price relative to the original?", c:["Same as original — 50% up and 25% down balance out","Lower than original — 100→150→75.00, a 25% net decrease","Higher than original — 100→150→112.50, a 12.5% net increase","Exactly 75% of the original"], a:2, e:"100 × 1.50 = 150. 150 × 0.75 = 112.50. Net change: +12.5%. The final price is 12.5% higher than the original. Markup then discount does not return to original.", trap_type:null },
      { p:null, q:"A population decreases by 10% each year for 2 years. Using 100 as a starting value, what is the final population?", c:["80 — subtracted 10% of the original twice","82 — from rounding each year upward","80 — from computing 100×0.80 in one step","81 — the second year's 10% applies to 90, not 100"], a:3, e:"Compound percentage: Year 1: 100×0.90=90. Year 2: 90×0.90=81. Not 80 — because the second 10% applies to 90, not 100. This is the compound vs. simple percentage distinction.", trap_type:null },
      ...
      { p:null, q:"'What percent of 40 is 30?' Using 100 as a mental anchor, you set up: 30/40 = x/100. What is x?", c:["65","75","80","70"], a:1, e:"30/40 = 0.75 = 75%. The proportion approach: 30 out of 40 corresponds to x out of 100. 30/40 × 100 = 75." , trap_type:null},
    ],
    M4:[
      { p:null, q:"You are testing answers for: x² - 5x + 6 = 0. Choices: 1, 2, 3, 4. Test x=2: 4-10+6=0 ✓. Test x=3: 9-15+6=0 ✓. Both work. What is happening?", c:["Nothing is wrong — this quadratic has two roots, x=2 and x=3","You made an arithmetic error — quadratics have only one solution","Test x=1 to find the real answer","The equation is set up incorrectly"], a:0, e:"Quadratics can have two solutions. Both 2 and 3 satisfy x²-5x+6=0. If the question asks 'what are the solutions,' both are correct. If it asks for 'the smaller solution,' x=2. Read the question carefully.", trap_type:null },
      { p:null, q:"The equation is: |2x - 4| = 6. Choices: -1, 1, 5, 7. Which choices work?", c:["Only x=5: |2(5)-4|=|6|=6 ✓","Only x=-1","Only x=1 and x=7","x=5 and x=-1 both work"], a:3, e:"Absolute value equations often have two solutions. Test all choices: x=5: |10-4|=6 ✓; x=-1: |-2-4|=6 ✓. Both are solutions. If the question asks for 'a solution,' either works. If it asks for 'the positive solution,' x=5.", trap_type:null },
      { p:null, q:"For which equation type is 'Solve Algebra Without Algebra' LEAST useful?", c:["Quadratic equations with integer roots","Complex rational equations","Equations whose choices are expressions, not numbers","Two-step linear equations with non-integer solutions"], a:2, e:"When choices are variable expressions (e.g., 'x²+3x' or '2n-1'), you cannot plug them back into the equation meaningfully. The strategy requires specific numerical answer choices to test.", trap_type:null },
    ...
    M5:[
      { p:null, q:"A diagram shows two triangles. The question gives angle values in the 30s and 40s and asks for a missing side. Choices are: 2, 8, 47, 180. Which can be immediately eliminated?", c:["47 and 180 — far out of proportion to the given values","None — all are possible in some geometric configuration","Only 180 — that's an angle, not a side","Only 2 — too small"], a:0, e:"Ballpark: triangle with angles 30-40°. The sides would be proportional and in a reasonable range. 47 and 180 are wildly disproportionate for a figure drawn at typical scale. Eliminate immediately.", trap_type:null },
      { p:null, q:"A question asks for the area of a circle inscribed in a square with side 10. You calculate π×10²=314. What is wrong?", c:["Nothing — π×r² with radius 10 gives the correct area","The formula should be circumference, 2πr=20π","The radius is 5, not 10 — the circle's area must be under the square's 100","The circle has diameter 10, but diameter and radius are the same"], a:2, e:"Ballpark check: the circle is inscribed IN the square (side=10) → radius=5 (NOT 10). Area=π×25≈78.5. A result of 314 is larger than the square (area=100) — physically impossible. Ballparking caught the error.", trap_type:null },
      ...
      { p:null, q:"A question asks for the number of students in a school with 12 classes of 25 students each. The choices are: 30, 300, 3000, 30000. Without precise calculation, which is correct?", c:["30 — barely more than one class's worth","3000 — ten times too large","30000 — a hundred times too large","300 — 12×25; ten classes ≈ 250, so 300 fits"], a:3, e:"12×25=300. Ballpark: 10×25=250; add 2 more classes of 25 = 300. Confirmed. 30 is too small (only one class's worth), 3000 and 30000 are too large by a factor of 10 or 100.", trap_type:null },
      { p:null, q:"A question asks for the speed of a car that travels 150 miles in 2.5 hours. Choices: 6, 60, 600, 6000. Which can be confirmed by quick ballpark?", c:["60 mph — typical highway speed; 150÷2.5=60","6 mph — walking pace, far too slow for a car","600 mph — the speed of a jet aircraft","6000 mph — orbital-rocket territory"], a:0, e:"Ballpark: 150 miles in 2.5 hours. At 60 mph: 60×2.5=150 ✓. Ballparking confirms — 60 mph is a realistic car speed, and the arithmetic checks out. 600 and 6000 are unrealistic for a car.", trap_type:null },
      { p:null, q:"You're solving a rate problem. Your answer is 450 hours. The problem involves a car traveling between two cities in a single trip. Is this reasonable?", c:["Yes — some car trips are very long","Yes — it depends on traffic","No — all car trip problems have answers under 10 hours","No — 450 hours ≈ 19 days of driving; recheck the setup"], a:3, e:"Sanity check with context: 450 hours = 18.75 days of continuous driving. This fails the common-sense test for a 'car trip between two cities.' Estimation flags the error even without recalculating.", trap_type:null },
    ...
    M6:[
      ...
      { p:null, q:"Which calculation correctly uses unit tracking to find how many hours it takes to drive 210 miles at 70 mph?", c:["70 mi/hr × 210 mi = time","210 mi × 70 mi/hr = time","70 mi/hr - 210 mi = time","210 mi ÷ (70 mi/hr) = 3 hours"], a:3, e:"Distance ÷ speed = time. 210 mi ÷ (70 mi/hr). Units: mi ÷ (mi/hr) = mi × (hr/mi) = hr ✓. Result: 3 hours.", trap_type:null },
      { p:null, q:"A tank fills at 3 liters per minute and drains at 1 liter per minute. How long (in minutes) to fill 20 liters?", c:["20 ÷ (3−1) = 10 minutes","20 ÷ 3 = 6.67 minutes","20 ÷ 1 = 20 minutes","20 × 3 = 60 minutes"], a:0, e:"Net rate = fill rate - drain rate = 3 - 1 = 2 L/min. Time = 20 L ÷ 2 L/min = 10 minutes. Unit tracking: L ÷ (L/min) = min ✓.", trap_type:null },
    ],
    M7:[
      { p:null, q:"A problem says: 'The sum of two consecutive even integers is 46. What are the integers?' You solve n+(n+2)=46 → n=22. The integers are 22 and 24. The question asks for the LARGER integer. The trap answer is:", c:["22 — the smaller","24 — the larger (correct)","23 — an odd integer (not even)","46 — the sum itself"], a:3, e:"The trap answers are: the smaller integer (22) and the sum (46). Students who solve for n but forget to add 2, or confuse the sum with the answer, pick 22 or 46. The question asks for the LARGER — re-read the last line before selecting.", trap_type:null },
      ...
      { p:null, q:"A problem asks for 'the value of x + 1' when 2x - 3 = 7. You solve: x = 5. Then you add 1: x + 1 = 6. The trap answer is:", c:["6","5 — stopping at x instead of x+1","3 — the constant in the equation","7 — the right side of the equation"], a:1, e:"The trap: stopping at x=5 and selecting it without computing x+1. This is the most common trap answer type on SAT Math: the question asks for an expression involving x, not x itself.", trap_type:null },
      { p:null, q:"After setting up a word problem, you get: apples = 12, oranges = 8. The question asks 'how many more apples than oranges?' You should:", c:["Report 12 as the answer — that's the main value solved for","Report 4 — the question asks for the difference, not the apple count","Report 8 as the answer — it's the smaller quantity","Report 20 (12+8=20) — total fruit"], a:1, e:"Re-read the last line: 'how many MORE' = difference. 12-8=4. Trap: 12 (number of apples, not the difference), 20 (total, not difference). The calculation is easy; the error is solving the wrong quantity.", trap_type:null },
    ...
    M8:[
      { p:null, q:"If p is any even integer, which of the following MUST be odd?", c:["p + 2 (even + even = even)","p × 2 (even × any = even)","p + 1 (even + odd = odd)","p² (even × even = even)"], a:2, e:"p is even. p+1 = even+odd = odd. Always true regardless of which even p you choose. p+2=even, p×2=even, p²=even. Only p+1 must be odd.", trap_type:null },
      { p:null, q:"If n is a negative integer, which MUST be positive?", c:["n + 1 (still negative if n < -1)","n × 2 (negative × positive = negative)","n² (negative × negative = positive)","n - 1 (more negative)"], a:2, e:"n²: any integer squared is non-negative. For a negative integer specifically, n×n = positive. n²>0 must be true. n+1 and n-1 remain negative (unless n=-1, where n+1=0, not positive). n×2 stays negative.", trap_type:null },
    ...
    M9:[
      { p:null, q:"Two lines on a coordinate plane appear to intersect at (3, 5). Your estimate from the graph is that the intersection is between x=2 and x=4. If the calculated intersection is at x=7.8, should you trust your calculation?", c:["Yes — calculated values are always more accurate than visual estimates","Yes — graphs at small scale exaggerate intersections","No — always discard the calculated value and use the visual estimate","No — 7.8 is far outside your 2–4 estimate; recheck the algebra"], a:3, e:"When your calculation contradicts a careful visual estimate, recheck the algebra. Visual estimates from graphs are rough but should be in the right ballpark. A result of 7.8 when the visual clearly shows ~3 is a red flag.", trap_type:null },
      { p:null, q:"A problem involves a triangle with vertices at (0,0), (6,0), and (3,4). You estimate the area visually as 'roughly 12.' You calculate ½×base×height = ½×6×4 = 12. What does this confirm?", c:["Estimate and calculation confirm each other","The formula is wrong — area should be larger","Visual estimates are unreliable for area","The base is not 6"], a:0, e:"When visual estimate and calculation agree, you have two independent lines of confirmation. ½×6×4=12 and the rough visual both yield approximately 12. This is how estimation serves as error-checking.", trap_type:null },
      { p:null, q:"A circle with radius 5 is drawn on a grid. A student estimates its area as 'somewhat less than the 10×10 square that contains it.' The calculated area is 25π ≈ 78.5. The square's area is 100. Is 78.5 a reasonable answer?", c:["No — the circle's area should be much less than 78.5","No — the area should equal the square's area","Yes — an inscribed circle fills ~78.5% of its square","Yes — but only if the radius equals 5"], a:2, e:"25π ≈ 78.5 ≈ 78.5% of the 10×10 square (area 100). A circle inscribed in a square always takes up π/4 ≈ 78.5% of the square's area. This is a classic geometric relationship. The answer matches the estimate.", trap_type:null },
    ],
    M10:[
      { p:null, q:"You graph y = x² - 6x + 8 in Desmos. What can you read directly from the graph without any calculation?", c:["Only the single y-intercept value","Only the vertex point","The zeros, the vertex, and the y-intercept","The value of the equation's discriminant"], a:2, e:"Desmos shows all key features visually: x-intercepts (where the curve crosses the x-axis), vertex (lowest/highest point), and y-intercept (where it crosses the y-axis). All readable on first graph, no algebra needed.", trap_type:null },
      { p:null, q:"To find the zeros of y = 2x³ - 5x² - 4x + 3 using Desmos, what is your process?", c:["Graph it in Desmos and read where the curve crosses the x-axis","Use the quadratic formula for cubic equations","Find the y-intercept (x=0) and work backwards","Type the equation into a calculator app and read the result"], a:0, e:"Desmos for zeros: graph the function, look where it crosses the x-axis. For a cubic, there may be 1, 2, or 3 crossings. Each crossing is a zero. No formula needed — just read coordinates from the graph.", trap_type:null },
      { p:null, q:"You need to find where f(x) = x² + 3x - 10 equals zero. You know factoring should work, but you are unsure of the factors. The fastest approach on the SAT with Desmos:", c:["Spend time trying to factor: (x+?)(x-?)","Use the quadratic formula: time-consuming but reliable","Substitute x=0 and solve from there","Graph it in Desmos and read the x-intercepts instantly"], a:3, e:"Desmos: type the function, read x-intercepts. For x²+3x-10: graph shows x=2 and x=-5 immediately. Much faster than factoring attempts or applying the quadratic formula with arithmetic risk.", trap_type:null },
    ...
    M11:[
      ...
      { p:null, q:"A system of two equations has no solution. What does the Desmos graph look like?", c:["Two lines that cross at the origin","Two parallel lines that never intersect","Two lines that overlap completely","Two curved lines"], a:1, e:"No solution = parallel lines. They have the same slope but different y-intercepts — they run alongside each other, never crossing. Infinitely many solutions = the same line plotted twice (they overlap).", trap_type:null },
      ...
      { p:null, q:"When graphing a system in Desmos, you see the two lines appear to be parallel. Should you trust this visual?", c:["Yes — always trust Desmos visuals over calculation","No — zoom in; near-parallel lines can cross outside the visible window","Yes — if they look parallel at default zoom, they are parallel","No — Desmos cannot show parallel lines"], a:1, e:"Two lines with very similar but not identical slopes may appear parallel at one zoom but cross far outside the window. To confirm no intersection, either zoom out dramatically or check algebraically whether the slopes are truly identical.", trap_type:null },
    ],
    M12:[
      ...
      { p:null, q:"You are asked to find k such that f(x) = k·e^x passes through (0, 5). What is k without Desmos?", c:["k = 5","k = 5e","k = e/5","k = 0"], a:0, e:"e^0=1 for any value of e. So f(0)=k×1=k=5. This is a case where algebra is faster than Desmos — direct substitution gives the answer instantly without opening a slider.", trap_type:null },
      { p:null, q:"The slider method in Desmos is most valuable for which type of problem?", c:["Any problem involving a constant or parameter, regardless of complexity","All percentage and ratio problems","Complex functions where isolating the parameter algebraically is slow","Problems containing only linear functions"], a:2, e:"Sliders work best when algebraic isolation of the parameter is slow or complex. For simple f(x)=kx, just substitute. For complex expressions involving multiple operations, sliders let you tune k visually in seconds.", trap_type:null },
      { p:null, q:"You graph y = k·sin(2x) and drag the slider until the graph passes through (π/4, 3). The slider lands at k=3. What does this mean?", c:["k is approximately 3 — the slider is imprecise","k=3 exactly — sin(π/2)=1, so y=k","k=3 only if x=π/4","The value of k depends on which window you are viewing"], a:1, e:"At x=π/4: sin(2×π/4)=sin(π/2)=1. So y=k×1=k. To pass through y=3, k=3 exactly. The slider shows 3, and algebra confirms it. When sin or cos evaluates to ±1 at the test point, the arithmetic becomes trivial.", trap_type:null },
      { p:null, q:"After finding a parameter k using the slider method, the answer choices include exact fractions (e.g., 2/3, 7/4) while your slider shows approximately 0.667. What should you do?", c:["Select the closest decimal approximation","Re-do the problem algebraically","Recognize 0.667 ≈ 2/3 and select 2/3","The slider is wrong if it shows a decimal"], a:2, e:"Sliders show decimals; answer choices may be exact fractions. 0.667 ≈ 2/3. When your slider value is close to a simple fraction in the choices, that fraction is the intended exact answer. Select it.", trap_type:null },
    ],
    M13:[
      { p:null, q:"You graph f(x) = (x+2)(x-3) and g(x) = x² - x - 6 in Desmos. The graphs are identical. This means:", c:["Both functions happen to look similar at this zoom level","They are equal only for x values between -2 and 3","The expressions are algebraically equivalent","The calculator made an error"], a:2, e:"Identical graphs = equivalent expressions. (x+2)(x-3) = x²-3x+2x-6 = x²-x-6. Desmos confirms the algebraic equivalence by showing the same curve for both.", trap_type:null },
      { p:null, q:"You plug x=3 into two expressions: A = (x-1)² - 4 and B = x² - 2x - 3. Both give 0. Can you conclude they are equivalent?", c:["Yes — agreement at x=3 confirms equivalence","No — the only way to confirm equivalence is algebraic proof","Yes — if they agree at x=3 which is in the domain","No — one point is not enough; graph both or test more points"], a:3, e:"One test point is not sufficient to prove equivalence — many different expressions can pass through the same point. Test at least 2-3 values or graph both to confirm. Here, A and B are actually equivalent: (x-1)²-4=x²-2x+1-4=x²-2x-3. ✓", trap_type:null },
    ...
    M15:[
      { p:null, q:"You need to simplify 17 × 23 - 17 × 3 mentally. Should you use Desmos?", c:["Yes — Desmos can compute this quickly","No — factor: 17(23−3) = 17×20 = 340; faster than opening Desmos","Yes — any multiplication with two-digit numbers benefits from Desmos","No — this type of calculation cannot be done mentally"], a:1, e:"Factor: 17(23-3)=17×20=340. Mental math with factoring is faster than navigating to Desmos for a pure arithmetic calculation. Know when to use Desmos (complex functions, graphs, systems) vs. mental math (arithmetic).", trap_type:null },
      { p:null, q:"Which SAT problem type benefits MOST from Desmos?", c:["Compute 15% of 60 for a store discount problem","Compute the product of the integers 12 and 15","Find all values of x where f(x) = x³ - 3x² - x + 3 equals zero.","Compute 7 × 8 in a single arithmetic step"], a:2, e:"A cubic can have 3 zeros — finding them algebraically (synthetic division or trial-and-error) is slow. Graphing in Desmos shows all zeros instantly. The arithmetic problems (A, B, D) are faster to compute mentally.", trap_type:null },
      { p:null, q:"You have 2 minutes left with 3 questions remaining. Question 25 asks for the vertex of y = 3x² - 12x + 7. With 2 minutes for 3 questions, should you use Desmos?", c:["Yes — always use Desmos for parabola problems","Either — both take ~30 seconds; pick the one you trust more","No — never use Desmos for vertex problems","Yes — Desmos is always faster than vertex formula"], a:1, e:"With 40 seconds per question, both approaches are viable. Vertex formula: h=-b/2a=12/6=2; k=3(4)-24+7=-5. Vertex: (2,-5). Desmos: graph the function, read the vertex. Choose the approach you trust more at this point in the test.", trap_type:null },
      ...
      { p:null, q:"You are about to open Desmos for a problem. When should you pause and consider whether algebra would be faster?", c:["Never — Desmos is always the better choice","When the algebra takes under 15 seconds","Always — algebra is always faster than Desmos","When the problem involves any polynomial"], a:1, e:"Desmos overhead (opening, typing) = 15-30 seconds. If the algebraic solution takes under 15 seconds, skip Desmos. The break-even point: use Desmos when the function is complex enough that setting it up algebraically would take longer than graphing it.", trap_type:null },
    ],
    // ── BATCH 4: Charts & Data (C1–C4) + Mixed Number / Misc (MN1–MN5) ──
    C1:[
      { p:null, q:"A bar chart shows monthly sales: Jan=40, Feb=55, Mar=70, Apr=60. What is the average monthly sales across these four months?", c:["55","56.25","57.5","60"], a:1, e:"(40+55+70+60)/4 = 225/4 = 56.25. Reading all bars accurately is the first step; computing the average is the second. Neither step is hard, but both must be done correctly." , trap_type:null},
      { p:null, q:"A pie chart shows: Category A = 40%, B = 30%, C = 20%, D = 10%. The total quantity is 500 units. How many units are in Category C?", c:["20","100","10","200"], a:1, e:"20% of 500 = 0.20 × 500 = 100 units. Reading the percentage from the chart (20%) and applying it to the total (500) are the two steps. Don't confuse the percentage label with the actual count." , trap_type:null},
      ...
      { p:null, q:"A two-way table shows: of 80 students who studied, 60 passed; of 20 students who did not study, 5 passed. What is the pass rate for students who studied?", c:["75%","65%","80%","60%"], a:0, e:"60 passed out of 80 who studied: 60/80 = 0.75 = 75%. Two-way table reads: find the appropriate row or column, identify the relevant counts, compute the ratio. Don't mix rows — the non-study rate is 5/20=25%, not relevant here." , trap_type:null},
    ],
    C2:[
      { p:null, q:"A histogram shows test scores with bars at: 60-70 (height 5), 70-80 (height 12), 80-90 (height 18), 90-100 (height 7). The mode interval is:", c:["70-80","80-90","60-70","90-100"], a:1, e:"Mode in a histogram = the interval with the tallest bar. 80-90 has height 18 — taller than all others. The mode interval contains the most data points." , trap_type:null},
      { p:null, q:"A box plot shows: minimum=20, Q1=35, median=50, Q3=65, maximum=80. The interquartile range (IQR) is:", c:["60","30","50","25"], a:1, e:"IQR = Q3 - Q1 = 65 - 35 = 30. The IQR represents the spread of the middle 50% of the data. It is read directly from the box in the box plot (the width of the box)." , trap_type:null},
    ...
    C3:[
      ...
      { p:null, q:"A researcher takes a random sample of 500 students from a school with 2,000 students. She finds that 60% prefer online classes. What is the best estimate for the number of all 2,000 students who prefer online classes?", c:["600","1200","500","800"], a:1, e:"Proportional reasoning: if 60% of the random sample prefer online classes, the best estimate for the full population is 60% × 2000 = 1200. Random sampling supports this extrapolation. Key word: 'random sample' — this validates the extrapolation." , trap_type:null},
    ...
    C4:[
      { p:null, q:"A data set has mean = 50 and standard deviation = 10. A value of 70 is how many standard deviations above the mean?", c:["1","2","3","20"], a:1, e:"Standard deviations from mean = (value - mean) / SD = (70-50)/10 = 2. This is a z-score calculation. 70 is 2 standard deviations above the mean." , trap_type:null},
    ...
    MN2:[
      ...
      { p:null, q:"A function f is defined by f(x) = 2x² - 3x + 1. What is f(-2)?", c:["11","15","3","9"], a:1, e:"f(-2) = 2(-2)² - 3(-2) + 1 = 2(4) + 6 + 1 = 8 + 6 + 1 = 15. Two common errors: (1) forgetting (-2)²=4, not -4; (2) forgetting -3×(-2)=+6, not -6. Sign discipline matters." , trap_type:null},
    ...
    MN3:[
      ...
      { p:null, q:"The ratio of boys to girls in a class is 3:5. If there are 25 girls, how many boys are there?", c:["20","15","18","12"], a:1, e:"3:5 ratio with 25 girls: set boys=3k, girls=5k. 5k=25 → k=5 → boys=3(5)=15. Verify: 15:25 = 3:5 ✓. Set up the ratio with a variable multiplier (k) rather than dividing directly — it keeps the answer a whole number and avoids the trap of forcing a non-integer result." , trap_type:null},
      { p:null, q:"A line has slope -2/3 and passes through (6, 1). What is its y-intercept?", c:["5","3","-3","1"], a:0, e:"y=mx+b. Substitute m=-2/3, point (6,1): 1=(-2/3)(6)+b → 1=-4+b → b=5. Equation: y=(-2/3)x+5. Verify at x=6: y=(-2/3)(6)+5=-4+5=1 ✓." , trap_type:null},
    ],
    MN4:[
      ...
      { p:null, q:"An arithmetic sequence has first term 7 and common difference 4. What is the sum of the first 10 terms?", c:["215","250","230","270"], a:1, e:"Arithmetic sum formula: Sₙ = n/2 × (a₁ + aₙ). aₙ = 7 + (10-1)×4 = 7+36 = 43. S₁₀ = 10/2 × (7+43) = 5 × 50 = 250." , trap_type:null},
    ...
    MN5:[
      ...
      { p:null, q:"The interior angles of a polygon sum to 1080°. How many sides does the polygon have?", c:["7","8","6","9"], a:1, e:"Formula: sum of interior angles = (n-2)×180. Set equal to 1080: (n-2)=1080/180=6 → n=8. An octagon." , trap_type:null},
