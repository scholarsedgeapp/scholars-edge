...
const StrategyCourseModule = (() => {
  ...
  const SECTIONS = {
    ...
    D:{ id:'D', name:'Grammar & Writing',      shortName:'Grammar',    icon:'type',        color:'#6C63FF', desc:'The SAT tests grammar through logic, not rule names. Ten pattern-recognition techniques.',                    codes:['G1','G2','G3','G4','G5','G6','G7','G8','G9','G10'] },
  ...
  const STRATEGIES = {
    ...
    G1:{ code:'G1', name:'Pattern Recognition Over Rule Naming', section:'D',
      ...
      explain:'Students freeze on grammar questions they could answer, because they think they need the rule\'s name. You don\'t. The SAT tests grammar through consistency: the text around the blank establishes a pattern — a register, a repeated frame, a form it has already committed to — and the right answer continues it. "Stow your tray. Fasten your belt. ___ your seat upright" — you can hear that the blank wants another bare command, without naming any rule. Pattern Recognition is permission to trust that ear: find what the text is already doing, and pick the choice that keeps doing it. (Specific pattern families — parallel lists and comparisons, word choice — get their own dedicated lessons in G9 and G10; this is the umbrella instinct behind them all.)',
      ...
      example:{ context:'Grammar · Pattern Matching', passage:null,
        q:'The safety card\'s instructions share one form: "Stow your tray. Fasten your belt. [BLANK] your seat upright."',
        c:['Bring','You should bring','Bringing','Please be sure to bring'],
        a:0, steps:['Read what the card is already doing: two instructions, each a bare command — "Stow your tray. Fasten your belt."','Name the pattern in plain words: every instruction opens with a bare command verb. No textbook term required.','Choice B adds "You should," and choice D adds "Please be sure to" — both turn a command into advice, breaking the established shape. Eliminate.','Choice C, "Bringing," is a form that can\'t issue a command at all: "Bringing your seat upright" gives no instruction. Eliminate.','Choice A, "Bring," is a third bare command: Stow. Fasten. Bring. The card\'s pattern continues seamlessly.','The general instinct on display: the text establishes its own pattern, and the right answer keeps it. Specific families — list and comparison alignment (G9), word choice (G10) — get their own lessons; this reflex powers them all.'] }
    },
    ...
    G8:{ code:'G8', name:'Modifier Placement', section:'D',
      bands:[2,3,4,5,6,7], skills:['grammar'],
      explain:'English readers assign a describing phrase to the nearest thing it could describe — so the SAT writes sentences where the nearest thing is wrong. "Running for the bus, my backpack split open": the opening phrase must describe whoever was running, but the sentence hands it "my backpack." That\'s a dangling modifier. The rule: a modifier must touch what it modifies. When a sentence opens with a describing phrase, the noun right after the comma must be the thing described; and when a limiter like "only" or "nearly" floats loose, it must sit beside the exact word it limits.',
      why:'Modifier errors are invisible to the ear because YOU know who was running — your brain silently repairs the sentence before the grammar registers. The test exploits that repair reflex: it writes sentences that communicate perfectly while being built wrong, then offers one choice that puts the described thing where the modifier can reach it. The check is mechanical precisely because comprehension can\'t be trusted here.',
      trigger:'Any sentence opening with an -ing or -ed phrase followed by a comma — ask whether the noun after the comma is the thing described. Also any floating "only," "nearly," or "almost" whose position changes the meaning.',
      mistake:'Judging by comprehensibility: "I get what it means, so it\'s fine." Comprehension is the trap\'s cover — the meaning arrives even when the grammar fails. The test is proximity, not clarity: what does the phrase actually touch?',
      example:{ context:'Grammar · Modifiers', passage:null,
        q:'Weakened by decades of winter storms, [BLANK].',
        c:['engineers closed the pier for urgent repairs','the pier was closed by engineers for repairs','the closure of the pier was ordered by engineers','engineers ordered the pier closed for repairs'],
        a:1, steps:['Ask what the opening phrase describes: "Weakened by decades of winter storms" — something that can be weakened by storms. That\'s the pier.','The rule: the noun right after the comma must be the weakened thing. Now audit the choices for what actually sits there.','Choices A and D put "engineers" after the comma — the sentence now says the ENGINEERS were weakened by decades of storms. Comprehensible, and wrong. Eliminate.','Choice C puts "the closure" there — a closure can\'t weather storms either. Eliminate.','Choice B places "the pier" against the modifier: the weakened thing, touching its description.','Notice the trap\'s design: every wrong choice still communicates. Your repair reflex supplies the intended meaning — the proximity check is what catches the grammar.'] }
    },
    G9:{ code:'G9', name:'Parallel Structure', section:'D',
      bands:[1,2,3,4,5,6,7], skills:['grammar'],
      explain:'When a sentence lines ideas up — in a list, a comparison, or a paired frame — the aligned parts must wear the same grammatical form. "She loves hiking, painting, and to sail" breaks because the series established -ing forms and then switched shapes. Three triggers to know: lists (A, B, and C), comparisons (more X than Y, X rather than Y), and correlative pairs (not only/but also, either/or, both/and, neither/nor). In each one, find the anchor — the first element, or the phrase after the frame\'s opener — and make every partner match its form exactly.',
      why:'Parallelism is the grammar of alignment: matched forms tell the reader "these ideas have equal status." The test builds items by matching every element but one, because a single shape-change hides easily in a sentence that still makes sense. Meaning survives the break — form doesn\'t — so students who check "does it sound sensible?" miss what students who check shapes catch every time.',
      trigger:'Commas building a series; comparison hinges (than, rather than, as); correlative openers (not only, either, both, neither). Locate the anchor element and demand its exact form from every partner slot.',
      mistake:'Matching meaning instead of form. "Clearing brush, repairing fences, and they built bridges" describes three related activities — the meaning aligns perfectly while the third shape defects. The standard is grammatical form, checked visually, not topical fit.',
      example:{ context:'Grammar · Parallel Structure', passage:null,
        q:'The fellowship rewards researchers for publishing openly, for mentoring younger colleagues, and [BLANK].',
        c:['for engaging the public','engagement with the public','to engage the public','they engage the public'],
        a:0, steps:['Spot the trigger: commas building a series — this is a list, so parallelism governs the blank.','Find the anchor: "for publishing openly." The second element confirms it: "for mentoring younger colleagues." The list\'s shape is "for + -ing phrase."','Choice B swaps to a plain noun ("engagement") — the "for" pattern vanishes. Eliminate.','Choice C swaps to an infinitive ("to engage") — a legal verb form in some lists, but not THIS list\'s form. Eliminate.','Choice D breaks out of the series entirely with a new clause. Eliminate.','Choice A delivers a third "for + -ing": publishing, mentoring, engaging. Anchor found, forms matched — that\'s the entire rule.'] }
    },
    G10:{ code:'G10', name:'Diction Precision', section:'D',
      bands:[2,3,4,5,6,7], skills:['grammar'],
      explain:'Some wrong answers aren\'t ungrammatical — they\'re the wrong word. The SAT tests diction three ways: confusable pairs that sound alike but split jobs (affect/effect, than/then, fewer/less, its/it\'s), idioms that demand a specific preposition (responsible FOR, capable OF, prohibited FROM), and precision — choosing the word whose meaning exactly fits, not a vague cousin. The method: when the choices are single words or short phrases that all "fit the slot" grammatically, stop reading for grammar and start reading for meaning. What exactly does each candidate mean, and what exactly does this sentence need?',
      why:'Confusable pairs survive school because both words pass the ear — you\'ve heard "effect" and "affect" in the same sentences for years, so they merged into one fuzzy word. The test un-merges them: it isolates the one slot where only one of the pair works, and it does the same with prepositions and near-synonyms. These items are pure knowledge-plus-precision; no elimination trick substitutes for knowing each word\'s actual job.',
      trigger:'Choice sets of near-identical short words (affect/effect, than/then); a preposition slot after a fixed verb or adjective; or four rough synonyms where the sentence\'s logic demands one exact shade.',
      mistake:'Choosing by familiarity or frequency — "effect is the one I see more often." Each confusable has a job: affect is the verb (to influence), effect is the noun (the result), with rare exceptions. Learn the job, not the popularity, and re-derive it from the slot every time.',
      example:{ context:'Grammar · Word Choice', passage:null,
        q:'The new irrigation schedule had an immediate [BLANK] on the orchard\'s yield.',
        c:['affect','effect','effecting','affection'],
        a:1, steps:['Read the slot, not just the sentence: "had an immediate ___ on" — the article "an" plus the adjective means the blank must be a NOUN.','Assign each candidate its job: "affect" is the verb (to influence); "effect" is the noun (the result); "effecting" is a verb form; "affection" is fondness.','The slot demands a noun naming a result. "Affect" fails the part-of-speech test no matter how familiar it sounds. Eliminate.','"Effecting" can\'t follow "an immediate" at all, and "affection" changes the sentence into a romance. Eliminate both.','"Effect" is the noun meaning result: the schedule had an immediate effect. Slot analyzed, jobs assigned, done.','The generalizable move: confusables are never settled by ear or familiarity — they\'re settled by the slot\'s part of speech and each word\'s actual job.'] }
    },
    M1:{ code:'M1', name:'Backsolving', section:'E',
      bands:[1,2,3,4,5,6,7], skills:['linear_algebra','advanced_math'],
      explain:'When a question asks for a specific value and offers four numbers, you\'re not holding one problem — you\'re holding four candidate answers that can be TESTED. Backsolving plugs a candidate into the problem\'s conditions and checks whether everything holds. Start from a middle value: because the real test sorts numeric choices, a wrong middle guess still tells you which direction to walk, eliminating half the list in one test. Deriving the answer builds an equation; backsolving only verifies one — and verification is arithmetic you already trust.',
      why:'Solving forward requires two skills — building the equation and manipulating it — and each is an error opportunity under time pressure. Testing backward requires only substitution. The sorted choices turn the search into a binary hunt: middle first, then up or down. Two tests usually finish the job, and every test doubles as its own check.',
      trigger:'"What is the value of…" with four specific numbers as choices. The more tangled the equation looks, the stronger the signal — tangled setups are exactly the ones that are cheap to CHECK and expensive to solve.',
      mistake:'Starting at the first choice and testing in order — that wastes the sorted structure. Start middle, read the miss\'s direction, walk. (In this app the choices appear shuffled; the real test sorts them, so order them mentally before picking your starting point.)',
      example:{ context:'Math · Algebra', passage:null,
        q:'If 3x + 7 = 22, what is the value of x?',
        c:['3','5','7','9'],
        a:1, steps:['Recognize the format: one unknown, four sorted numbers. This is backsolving\'s home turf — no equation-solving required.','Start mid-range with 5: 3(5) + 7 = 15 + 7 = 22. Match on the first test.','Understand the machinery for when the first test misses: had you tested 3, you\'d get 16 — too small — which eliminates 3 AND tells you to move up the list.','Had you tested 7: 28, too large — eliminating 7 and 9 together. Each miss halves the field.','Compare the costs: solving forward means subtract 7, divide by 3 — two chances to slip. Substituting means one multiply, one add, and the check is built in.','The habit: sorted numeric choices are not four options; they\'re a search space with directions included.'] }
    },
    M2:{ code:'M2', name:'Plug In Numbers', section:'E',
      bands:[2,3,4,5,6,7], skills:['advanced_math','linear_algebra'],
      explain:'When the answer choices contain variables, the question is really asking "which formula is right?" — and formulas can be tested with concrete numbers. Pick a simple value (2, 3, or 5 — never 0 or 1), run the PROBLEM with it to get a target number, then run each CHOICE with the same value. The choice that hits the target is the formula. Abstract algebra becomes two rounds of arithmetic, and your specific number acts as a fingerprint that wrong formulas almost never match.',
      why:'Variable answer choices invite symbol manipulation, which is where most algebra errors live. A concrete number sidesteps the symbols entirely: you never simplify, expand, or factor — you evaluate. The craft is in the value chosen: 0 and 1 have special properties (multiplication vanishes, powers collapse) that let several wrong formulas match the target, and too-round numbers can create coincidental ties. Ordinary values like 3 keep the fingerprint unique — and when two choices DO tie, a second value settles it.',
      trigger:'Answer choices containing n, x, k, or any variable. Also "in terms of" phrasing — that phrase literally announces a formula hunt.',
      mistake:'Plugging 0, 1, or a number from the problem itself, then trusting a single match. If two choices survive your first value, that\'s not failure — it\'s the signal to spend ten more seconds on a second value. One extra plug always breaks the tie.',
      example:{ context:'Math · Algebra', passage:null,
        q:'For any integer n, which expression equals (n + 2)² − n²?',
        c:['4','4n','4n + 4','2n + 4'],
        a:2, steps:['Variables in the choices: this is a formula hunt, so pick a fingerprint value. Take n = 3 (not 0 or 1 — their special properties let wrong formulas tie).','Run the problem: (3 + 2)² − 3² = 25 − 9 = 16. The target is 16.','Run the choices: A gives 4, B gives 12, C gives 4(3) + 4 = 16, D gives 10. One match.','Notice what never happened: no expanding (n + 2)², no cancelling n² — the errors those steps invite were never on the table.','If two choices had both hit 16, the move is a second value, not a coin flip: at n = 5 the problem gives 24, and only 4n + 4 keeps up.','Algebraic confirmation, if you want it: (n + 2)² − n² = 4n + 4. But the fingerprint had already decided.'] }
    },
    M3:{ code:'M3', name:'Plug In 100 for Percentages', section:'E',
      bands:[1,2,3,4,5,6,7], skills:['data_analysis'],
      explain:'Percent problems without a starting value are abstraction traps: "a price rises 20%, then falls 20%" invites you to reason about percents in your head, where they behave badly. Kill the abstraction by starting at 100. Every percent of 100 is just itself — 20% of 100 is 20 — so each step becomes plain arithmetic, and the final number IS the final percent. Chains of increases and decreases, reverse "what was the original?" questions, percents of percents: all of them flatten into two or three easy multiplications from a base of 100.',
      why:'The core percent illusion is that changes add: +20 then −20 "cancels," 30% of a group\'s 40% "is 70% or 10%." They don\'t and it isn\'t — each percent acts on the CURRENT base, which the previous step just changed. Reasoning about shifting bases abstractly is genuinely hard; watching them shift from a concrete 100 is trivial. The 100-start doesn\'t simplify the math — it reveals the base switches the traps rely on hiding.',
      trigger:'Any percent problem with no starting value given: "by what percent," "what percent of the original," chained increases and discounts, or a percent of a percent.',
      mistake:'Adding or subtracting the percents themselves (+25 then −20 is not +5) or applying a later percent to the original base instead of the current one. If you catch yourself doing percent arithmetic without a concrete number on the page, stop and restart from 100.',
      example:{ context:'Math · Percentages', passage:null,
        ...
        c:['0%','−4%','+4%','−2%'],
        a:1, steps:['No starting value given — that\'s the M3 signal. Put 100 on the page.','Step one: up 20%. From 100, that\'s +20 → 120. (This is why 100: the percent IS the number.)','Step two: down 20% — but of the CURRENT base, 120. That\'s −24, not −20 → 96.','Read the answer straight off: 96 against a start of 100 means −4%. No formula, no proportion setup.','Name the trap you dodged: 0% is the canceling illusion, built on the belief that the two 20s acted on the same base. The second one acted on 120.','Keep the pattern: every chained-percent question is this same three-line table from 100 — write it, don\'t reason it.'] }
    },
    M4:{ code:'M4', name:'Solve Algebra Without Algebra', section:'E',
      bands:[1,2,3,4,5,6,7], skills:['linear_algebra','advanced_math'],
      explain:'Some equations punish the "proper" route: quadratics that don\'t factor cleanly, cubics, exponent equations, word setups with two conditions. When the choices are specific numbers, you don\'t owe the equation a solution — you owe the question an answer, and substitution delivers it. Test choices directly in the original conditions. Where Backsolving (M1) walks sorted values with direction feedback, this is the blunter cousin: any messy equation, any candidate, checked by straight evaluation. The equation never gets solved. It gets satisfied.',
      why:'Factoring, formula-cranking, and multi-step isolation are the highest-error operations in SAT math, and they\'re also optional whenever candidates exist. Substitution replaces derivation with evaluation — a downhill skill. Word problems benefit doubly: a tested candidate checks ALL the stated conditions at once (area AND the length-width relationship), while an algebraic solve verifies them one equation at a time.',
      trigger:'Messy-to-factor quadratics and cubics, exponent equations, and multi-condition word problems — whenever the choices are concrete numbers. If your first factoring attempt stalls, that stall IS the signal.',
      mistake:'Finishing the algebra out of pride after the strategy has already answered the question — or trying to "test" choices that contain variables (circular; that\'s M2\'s job, with your own plugged number).',
      example:{ context:'Math · Quadratic', passage:null,
        q:'For which value of x is x² − 5x + 6 = 0?',
        c:['x = 1','x = 2','x = 4','x = 6'],
        a:1, steps:['Assess the fork: factor x² − 5x + 6, or evaluate it four times at most. Evaluation is error-proof; take it.','Test x = 1: 1 − 5 + 6 = 2. Not zero. Eliminate.','Test x = 2: 4 − 10 + 6 = 0. Satisfied — done, about twenty seconds in.','Note what "satisfied" means: the equation was never solved, factored, or rearranged. The candidate simply made it true, which is all the question asked.','The footnote that builds trust: algebraically x = 3 also works, but it isn\'t offered. The choices only ever contain one satisfying value — the test hands you a finite search space on purpose.','File the pairing: numeric choices + ugly equation = M4. Variable choices = M2. Sorted values with a hunt = M1. Three tools, one family.'] }
    },
    M5:{ code:'M5', name:'Answer Choice Ballparking', section:'E',
      bands:[1,2,3,4,5,6,7], skills:['linear_algebra','advanced_math','data_analysis'],
      explain:'Before any precise work, spend ten seconds deciding what SIZE the answer must be: round the numbers to friendly ones, bound the quantity with something you know, and look at the choices through that window. Two things happen. First, choices outside the window die instantly — on many SAT items that\'s two or three of the four, and on "closest to" items it\'s the entire question. Second, you\'ve pre-committed to a range, so if your precise calculation later lands outside it, you catch the error the moment it happens instead of submitting it.',
      why:'Calculators and formulas fail loudly but convincingly: a slipped decimal or a forgotten ½ produces a confident, precise, wrong number. The only defense that works at test speed is an independent estimate made BEFORE the calculation — because after it, your brain anchors on the computed number and rationalizes it. Estimation is also the test\'s own design language: choices are typically spread wide apart precisely so that size-checking can work.',
      trigger:'Choices spanning very different sizes or orders of magnitude; any "closest to" or "approximately" stem; every geometry item (the described figure bounds the answer); and always as the final pre-submit check.',
      mistake:'Calculating first and estimating never — or treating a wildly implausible result as "what the formula gave." The formula didn\'t give it; a slip did, and the size-check would have caught it before the answer sheet does.',
      example:{ context:'Math · Geometry', passage:null,
        ...
        a:1, steps:['Bound before computing: an inscribed circle can\'t be taller than the rectangle\'s short side, so its diameter is at most 5 and its radius at most 2.5.','Estimate the area from the bound: π × 2.5² ≈ 3 × 6.25 ≈ 19. The answer must live near 19 and cannot exceed the rectangle\'s 60.','Read the choices through the window: 78.5 is bigger than the whole rectangle — impossible. 50 crowds the rectangle\'s area — the corners forbid it. 9.8 is half the estimate.','Only 19.6 sits in the window. The "calculation" was one rough multiply.','Name the planted errors: 78.5 is the area if the radius were 5 (the short SIDE used as radius); 9.8 is a halving slip; 50 ignores the inscription constraint.','Keep the order of operations: window first, precision second — precision can only confirm or contradict a window that already exists.'] }
    },
    M6:{ code:'M6', name:'Units-First Analysis', section:'E',
      bands:[1,2,3,4,5,6,7], skills:['data_analysis','linear_algebra'],
      explain:'Rate and conversion problems are unit problems wearing number costumes. Before touching the numbers, read the answer choices to learn the unit you must END with, then write the chain that gets there: miles per hour times hours gives miles; grams divided by grams-per-batch gives batches. Each conversion is a fraction whose units cancel — write them, watch them cancel, and the arithmetic sets itself up. If the units don\'t cancel to the target, the setup is wrong no matter how right the numbers feel.',
      why:'The test plants its rate traps at the seams between units: minutes multiplied against per-hour rates, kilograms fed into per-gram formulas, conversions applied upside-down. Numbers can\'t warn you about any of that — 24 × 40 computes happily whether or not it means anything. Units object immediately: the chain refuses to cancel, or the direction check fails (a foot must cost LESS than a yard). Writing units first turns every one of those planted seams into a visible, checkable step.',
      trigger:'Any problem mixing time units, metric prefixes, money-per-quantity, or asking for an answer in different units than the given rate. Read the choices\' units FIRST — they are the destination.',
      mistake:'Multiplying the two numbers the problem hands you because they\'re there. Mixed-unit numbers multiplied raw produce a trap answer that IS listed — the test plants the unconverted result on purpose. A matching choice is not confirmation.',
      example:{ context:'Math · Rates', passage:null,
        ...
        a:1, steps:['Destination first: the choices are in MINUTES. The given data is in miles and hours — so the chain must end with a hoursminutes conversion. Plan it before computing.','Build the speed: 150 mi ÷ 2.5 hr = 60 mi/hr. Units: miles over hours. ✓','Time for 30 miles: 30 mi ÷ 60 mi/hr — miles cancel, leaving 0.5 HOURS. The units did the bookkeeping.','Finish the chain: 0.5 hr × 60 min/hr — hours cancel, leaving 30 minutes. Destination reached in the destination\'s units.','Check the seam you didn\'t fall into: answering "0.5" (hours, unconverted) or dividing the wrong way (60 ÷ 30 = 2) — both produce numbers that look like times.','The discipline: every fraction you write carries its units, and every cancellation is a verification. When the units arrive at the target, the number riding with them is the answer.'] }
    },
    M7:{ code:'M7', name:'Trap Answer Arithmetic', section:'E',
      bands:[2,3,4,5,6,7], skills:['linear_algebra','advanced_math','data_analysis'],
      explain:'SAT math choices are not four random numbers — they are the right answer plus three manufactured mistakes. The wrong choices are computed in advance from the most common slips: stopping one step early (x when the question asked for 2x + 1), using the wrong base or the wrong half (diameter as radius), running the inverse operation, echoing an intermediate total. Two powers come from knowing this. Defensively: your answer appearing among the choices proves nothing, because the mistakes appear there too. Offensively: if you can predict a problem\'s likely slip, you can often name which choices are traps before solving — and check your own result against the trap you\'d most plausibly have made.',
      why:'Test writers build distractors by actually making the errors: they solve the problem wrong in the three most statistically common ways and print the results. That\'s why a predictable mistake always "finds" a matching choice, and why match-euphoria is so dangerous. But the same design is exploitable — the traps are a map of the problem\'s danger zones, drawn by the test itself.',
      trigger:'Every math question, in two moments: before solving (what\'s the likely slip here — early stop, wrong base, missed conversion?) and before submitting (does my number match the slip I predicted?). Highest alert when your answer arrives suspiciously fast.',
      mistake:'Submitting on match: "my number is choice C, so I\'m done." The strongest single defense is re-reading the question\'s final line after solving — most planted traps answer a different question than the one actually asked.',
      example:{ context:'Math · Number Properties', passage:null,
        ...
        c:['22','23','24','25'],
        a:2, steps:['Predict the trap before solving: multi-step setups plant the INTERMEDIATE value. Here, solving will produce the smaller integer first — expect it among the choices.','Solve: n + (n + 2) = 46, so 2n = 44 and n = 22. The integers are 22 and 24.','Re-read the final line: "the LARGER integer." Answer: 24.','Audit the choices against named errors: 22 is the early-stop trap (n itself, exactly as predicted). 23 comes from splitting 46 in half — consecutive-integer logic applied to an even-integer problem. 25 is 23\'s partner.','Notice the defensive win: a student who solves correctly to n = 22 and submits on match hands the test a point. The trap was computed for precisely that student.','The habit pair: predict the slip going in, re-read the ask coming out. Between them, nearly every planted trap loses its target.'] }
    },
    M8:{ code:'M8', name:'Properties Over Procedures', section:'E',
      bands:[2,3,4,5,6,7], skills:['advanced_math','linear_algebra'],
      explain:'Some questions aren\'t asking you to compute anything — they\'re asking whether you know how numbers BEHAVE. "Must be even," "must be negative," "which is divisible by 3": these are property questions, and properties answer them in one line. The reference card is short: odd × odd = odd, even × anything = even, odd + odd = even; negative × negative = positive; a number is divisible by 3 when its digit sum is; squares are never negative; primes above 2 are odd. Apply the rule first, then confirm with one plugged value — the pair takes seconds and is nearly slip-proof.',
      why:'Procedure is the wrong tool here by design: "must be true for ALL integers n" can\'t be settled by any amount of expansion, because expansion shows one case at a time — exactly what "all" forbids. Properties quantify over every case at once; that\'s their whole job. The test writes these items knowing procedure-minded students will burn minutes deriving what a parity rule states instantly.',
      trigger:'"Must be," "could be," "always," "never" attached to number types — odd, even, positive, negative, prime, divisible. Also any divisibility check where long division looms: the digit-sum and last-digit rules exist for exactly that.',
      mistake:'Expanding algebraically to "prove" a must-be claim, or testing one example and declaring "must." One value can support a COULD; only a property (or a counterexample hunt) settles a MUST.',
      example:{ context:'Math · Number Properties', passage:null,
        q:'If p is odd and q is even, which must be even?',
        c:['p + q','p − q','p × q','p²'],
        a:2, steps:['Classify the question: "must be even" with number types — a property item. Reach for parity rules, not calculation.','Run the rules down the choices: odd + even = odd. Odd − even = odd. Odd × even = even. Odd² = odd × odd = odd.','One choice survives: p × q. The even factor forces the product even, always — that\'s the property doing the quantifying that "must" requires.','Confirm with one value pair, as a slip-check only: p = 3, q = 4 gives 7, −1, 12, 9. Matches the rules.','Understand why the plug alone wasn\'t enough: 3 and 4 show p × q CAN be even; the parity rule shows it MUST be. The example checks the rule; the rule carries the proof.','Keep the card current: parity, signs, divisibility shortcuts, prime facts. Ten rules cover essentially every property item the test writes.'] }
    },
    M9:{ code:'M9', name:'Geometry Visual Estimation', section:'E',
      bands:[1,2,3,4,5,6,7], skills:['data_analysis'],
      explain:'Every geometry setup carries built-in bounds, and the bounds do half the work before any formula starts. A hypotenuse is longer than either leg but shorter than their sum. Nothing inside a square exceeds its diagonal. An inscribed circle fills most — never all — of its box. A triangle is half its bounding rectangle. Sketch the described figure (on the digital SAT, figures are approximately to scale unless marked otherwise — and described figures you draw yourself are information too), read off the bounds, and eliminate every choice that violates them. Often one choice survives; always, your eventual calculation has a reality check waiting.',
      why:'Geometry formulas fail in signature ways — the forgotten ½, the diameter used as radius, the hypotenuse wandering into an area slot — and each failure produces a specific wrong size. Bounds catch wrong sizes without knowing which slip produced them. The picture is a second, independent solution path running in parallel with your algebra, and it\'s the cheaper of the two.',
      trigger:'Every geometry item: before computing, extract one bound from the figure (longest side, containing shape, half-of-rectangle). After computing, ask whether the result fits the picture.',
      mistake:'Calculating blind and trusting the output — "that\'s what the formula gave." Formulas transmit slips as confidently as truths; the picture is what refuses to. If the number doesn\'t fit the figure, the number is wrong, every time.',
      example:{ context:'Math · Geometry', passage:null,
        q:'A right triangle has legs of length 6 and 8. What is its area?',
        c:['14 sq units','24 sq units','48 sq units','10 sq units'],
        a:1, steps:['Sketch the description: legs 6 and 8 meeting at the right angle. The triangle is half of a 6 × 8 rectangle — that\'s the bound: area must be well under 48.','Compute inside the bound: ½ × 6 × 8 = 24. Exactly half the rectangle, as the picture promised.','Audit the choices as named slips: 48 is the rectangle — the forgotten ½, geometry\'s most-planted trap.','10 is the hypotenuse (6-8-10 triple) posing as an area; 14 is the legs\' sum. Both are real numbers from the figure sitting in the wrong slot.','Notice the two-lane verification: the formula said 24, and independently the picture said "half of 48." When both lanes agree, submit with confidence.','The habit: bound first, formula second, fit-check last. Three glances, and every signature slip in geometry becomes visible.'] }
    },
  ...
  const DRILLS = {
    ...
    G1:[
      { p:null, q:'The auditor\'s report stays formal throughout. Which choice matches the sentence\'s established register? "The shortfall [BLANK] three departments before it was detected."', c:['persisted across','dragged on through','hung around in','stuck around across'], a:0, e:'The surrounding prose sets the pattern: formal, measured, technical. "Persisted across" belongs to that register; the other three are conversational idioms that snap the tone. No rule name exists for this — it\'s pure pattern-matching against the sentence\'s own voice.' },
      ...
      { p:null, q:'The lab\'s door signs all share one form: "Badge in before entry. Gown up before the bench. [BLANK] before leaving."', c:['You must sanitize','Sanitizing','Sanitize','It is required to sanitize'], a:2, e:'Two signs establish the frame: bare command, then "before X." The third sign must echo it: "Sanitize before leaving." The longer choices say the same thing while abandoning the shape the signs already set — the mismatch is audible without any terminology.' },
      { p:null, q:'As the tide rises, the channel narrows; as the wind shifts, the ferry [BLANK].', c:['would slow','will have slowed','is slowing down','slows'], a:3, e:'The sentence built a frame and used it once: "As X rises, Y narrows." The second half repeats the frame, so the verb matches its shape: "slows." The other forms import tenses the pattern never established. (A time-anchor rule could get you here too — see G7 — but the frame\'s echo answers faster.)' },
      { p:'A student reads a formal report: "The committee\'s findings, gonna be released Thursday, describe systemic delays." She senses something is off but can\'t cite a rule, so she leaves it.', q:'What does Pattern Recognition tell her?', c:['Trust the mismatch: "gonna" breaks the register the sentence itself established','Leave the whole sentence alone, since no nameable rule means there is no error to fix','The problem is sentence length, not any pattern she could identify','Formal reports may mix registers when quoting a timeline'], a:0, e:'Every other word in the sentence wears formal dress; "gonna" walks in wearing flip-flops. The fix ("to be released Thursday") needs no rule name — the sentence\'s own register was the pattern, and her ear caught the break. That instinct is the strategy.' },
    ],
    G2:[
    ...
    G8:[
      { p:null, q:'Hoping to beat the morning heat, [BLANK] before sunrise.', c:['the hikers started up the trail','the trail was climbed by the hikers','the trailhead parking lot filled','it made sense to start hiking'], a:0, e:'The opening phrase describes someone who can hope — the noun after the comma must be that someone. "The hikers" is. The trail, the parking lot, and "it" can\'t hope for anything; each of those choices leaves the modifier dangling.' },
      { p:null, q:'Which revision fixes the error in "The bakery sold pastries to commuters filled with almond cream"?', c:['The bakery sold pastries to commuters that were filled with almond cream','The bakery sold pastries filled with almond cream to commuters','Filled with almond cream, the bakery sold pastries to commuters','The bakery, filled with almond cream, sold pastries to commuters'], a:1, e:'As written, "filled with almond cream" touches "commuters" — the wrong noun. The fix moves the phrase next to "pastries," the thing it describes. Choice A keeps the distance (and "that were" still reaches for commuters); C and D relocate the filling to the bakery itself.' },
      { p:null, q:'The librarian intends to say that no day except Friday works. Which placement of "only" says it? "The reading room [BLANK]."', c:['only can be reserved on Fridays','can only be reserved on Fridays','can be only reserved on Fridays','can be reserved only on Fridays'], a:3, e:'"Only" limits whatever it touches. The intended limit is the DAY, so "only" belongs beside "on Fridays." The other placements limit the wrong things — what can happen to the room, or the reserving itself — and each drifts from the meaning the librarian intends.' },
      { p:null, q:'Marinated overnight in citrus and garlic, [BLANK] impressed even the head chef.', c:['the cooks\' preparation of the pork','the cooking method behind the pork','the recipe for the pork shoulder','the pork shoulder'], a:3, e:'What was marinated? The pork shoulder — so it must sit right after the comma. The preparation, the method, and the recipe weren\'t soaked in citrus; each of those choices leaves the opener describing the wrong noun, however sensible the sentence still sounds.' },
      { p:'A student reads: "Driving through the pass at dusk, the radio towers seemed to blink in sequence." It sounds fine to her, and she can picture the scene perfectly, so she marks it correct.', q:'What does Modifier Placement say she missed?', c:['Comprehension is the trap\'s cover: the towers weren\'t driving, so the opener dangles','Nothing at all; a sentence that communicates this clearly cannot contain a modifier error','The real problem is "at dusk," which should open the sentence instead','"Seemed" is too weak a verb to support an opening phrase of that length'], a:0, e:'Her mental picture supplied a driver the sentence never names — the repair reflex at work. Grammatically, the noun after the comma is "the radio towers," and towers don\'t drive. Dangling openers survive precisely because comprehension papers over them.' },
    ],
    G9:[
      { p:null, q:'The internship taught her to draft briefs, to manage filings, and [BLANK].', c:['negotiating with clerks','to negotiate with clerks','negotiation with clerks','that she should negotiate with clerks'], a:1, e:'The series anchors on full infinitives: "to draft… to manage… and ___." "To negotiate" matches the anchor\'s shape. The -ing form, the plain noun, and the clause each break alignment — different shapes for a slot the list already sized.' },
      { p:null, q:'Reviewers admired the memoir more for its restraint than [BLANK].', c:['it being lyrical','how lyrical it was','for its lyricism','because of the lyric quality'], a:2, e:'Comparisons balance on their hinge: "more for X than for Y." The anchor is "for its restraint," so the partner is "for its lyricism." The others change construction mid-comparison, which is exactly the mismatch "than" exposes.' },
      { p:null, q:'The festival will either move indoors [BLANK].', c:['or possibly a postponement until spring','or it might be postponed until spring','or postponing until spring','or postpone until spring'], a:3, e:'"Either" attached to a bare verb ("move"), so "or" must deliver a bare verb: "postpone." The correlative pair frames two slots of identical shape; a noun, a clause, or an -ing form in the second slot breaks the frame.' },
      { p:null, q:'The new policy not only shortens wait times [BLANK] staffing costs.', c:['but also reduces','but reducing also','and also reducing','but it also will reduce'], a:0, e:'"Not only" sits before a present-tense verb phrase ("shortens wait times"), so "but also" must introduce the same shape: "reduces staffing costs." The distractors rearrange the pair\'s words or shift the verb\'s form — the frame demands symmetry on both sides.' },
      { p:'A student reads: "Volunteers spent the summer clearing brush, repairing fences, and they built two footbridges." She senses the ending is heavier than the rest but calls it a style preference and moves on.', q:'What does Parallel Structure say about the "style preference" call?', c:['She is right, since series endings may legitimately vary for extra emphasis','It\'s a form break: two -ing phrases, then a clause abandons the anchor shape','The real problem is the comma before "and," which changes the series\' rhythm','The sentence errs only because "built" is a stronger verb than "clearing"'], a:1, e:'"Clearing… repairing… and they built" starts as an -ing series and ends as an independent clause — a shape change, not a style choice. The fix restores the anchor: "building two footbridges." Alignment is a rule the list set for itself.' },
    ],
    G10:[
      { p:null, q:'The express lane is reserved for carts with [BLANK] than fifteen items.', c:['less','lesser','fewer','least'], a:2, e:'Items are countable, and countables take "fewer." "Less" belongs to quantities you measure rather than count (less water, less time). "Lesser" ranks importance, and "least" is a superlative with nothing to outrank here.' },
      { p:null, q:'The harbor survey took longer [BLANK] the crew had budgeted, so the mapping phase started late.', c:['then','as','that','than'], a:3, e:'"Longer" sets up a comparison, and comparisons take "than." "Then" is a time word — the sound-alike trap — while "as" pairs with "as long as" and "that" builds a clause the sentence isn\'t making.' },
      { p:null, q:'The safety board found the operator [BLANK] the missed inspections.', c:['responsible for','responsible of','responsible to','responsible about'], a:0, e:'The idiom is fixed: responsible FOR a thing (answerable for it). "Responsible to" exists but takes a person or body one answers to, not the fault itself; "of" and "about" are prepositions the adjective never takes.' },
      { p:null, q:'Because the two witnesses gave [BLANK] accounts, the investigators re-interviewed them separately.', c:['different','conflicting','various','assorted'], a:1, e:'The sentence\'s logic needs accounts that clash — that\'s why separate re-interviews follow. "Conflicting" carries the clash; "different" is too weak to force the response, and "various"/"assorted" describe collections, not a two-way disagreement. Precision means matching the word to the sentence\'s exact need.' },
      { p:'A student hits "The drought will [BLANK] harvest schedules across the region" and picks "effect" because "effect shows up more often in print." The choices were "affect," "effect," "effectuate," and "affected."', q:'What does Diction Precision say about her tiebreaker?', c:['Frequency was a fine tiebreaker, since "effect" is indeed more common as a verb','Both words work here, so the shorter one should have won','Popularity isn\'t the job: the slot needs a verb meaning influence, "affect"','She should have eliminated "effectuate" first and then guessed'], a:2, e:'"Will ___" demands a verb, and the verb meaning "to influence" is "affect." "Effect" is the noun (an effect), a verb only in the rare "effect change." Her tiebreaker — print frequency — is exactly how merged confusables stay merged. Learn each word\'s job.' },
    ],
    M1:[
      { p:null, q:'If 3(x + 4) = 5x − 2, what is the value of x?', c:['7','3','5','14'], a:0, e:'Backsolve: order the choices, start mid-range. Test x = 7: 3(11) = 33 and 5(7) − 2 = 33. Match. The traps are manufactured mistakes: 3 comes from distributing 3(x + 4) as 3x + 4, 5 from adding the 2 instead of subtracting it, and 14 from stopping at 2x = 14 without dividing.' },
      { p:null, q:'A number n satisfies n² − 2n = 35. Which value of n works?', c:['5','7','9','6'], a:1, e:'Middle-out backsolving: try 6 or 7 first. 7² − 2(7) = 49 − 14 = 35. Done. The bait is 5 — a factor of 35 that feels destined — but 25 − 10 = 15. Substitution beats intuition here: twenty seconds, no factoring required.' },
      { p:null, q:'A gym charges a $25 signup fee plus $18 per month. If Leah has paid $187 in total, for how many months has she been a member?', c:['10','8','9','12'], a:2, e:'Backsolve with 9: 25 + 18(9) = 25 + 162 = 187. Match. The traps: 10 ignores the signup fee (187 ÷ 18, rounded), 8 subtracts the fee and then one extra month, 12 adds the fee instead of subtracting it. Each wrong choice is a specific bookkeeping slip — plug-back exposes all of them.' },
      { p:null, q:'If √(2x + 7) = 5, what is the value of x?', c:['−1','6','16','9'], a:3, e:'Test x = 9: 2(9) + 7 = 25, and √25 = 5. The traps: −1 skips the squaring step entirely (treating 2x + 7 = 5), 16 squares correctly but adds the 7 instead of subtracting it, and 6 averages its way there from 5 + 7. Backsolving sidesteps every one of those slips.' },
      { p:'A student sees "Which value of k satisfies k³ + k = 130?" with choices 3, 4, 5, and 6. She starts building a factoring approach for the cubic.', q:'What does Backsolving say about this moment?', c:['Numeric ordered choices with one unknown is the signal: test 4 or 5 first','Cubic equations can never be reliably solved by testing the answer choices','Factoring is the safer route because plugging in can produce false positives','She should graph the cubic function first to locate the root visually'], a:0, e:'k = 5 gives 125 + 5 = 130 on the second or third test — under a minute, and no cubic factoring exists that\'s faster. Ordered numeric choices with a single unknown are backsolving\'s home turf; the format was the strategy hint.' },
    ],
    M2:[
      { p:null, q:'A workshop has n tables. Each table seats 4 people except the last, which seats 2. How many people can be seated, in terms of n?', c:['4n','4n − 2','4n + 2','2n + 4'], a:1, e:'Plug n = 3: two full tables (8) plus the last table\'s 2 = 10. Testing: 4n − 2 = 10 ✓ — but 2n + 4 = 10 also survives! That collision is the lesson: when two choices pass, change the number. At n = 5, seating = 18; 4n − 2 = 18 ✓ while 2n + 4 = 14 ✗. One extra plug settles it.' },
      { p:null, q:'Which expression is equivalent to (3x + 6)/3 + x?', c:['x + 2','x + 6','2x + 2','2x + 6'], a:2, e:'Let x = 2: (6 + 6)/3 + 2 = 4 + 2 = 6. Only 2x + 2 gives 6. The traps are partial simplifications: x + 2 divides but drops the added x, 2x + 6 forgets to divide the 6, and x + 6 divides only the first term. Plugging in catches all three without touching the algebra.' },
      { p:null, q:'The price of a jacket is p dollars. After a 30% discount and then a $10 coupon, the final price in dollars is:', c:['p − 40','0.3p − 10','0.7(p − 10)','0.7p − 10'], a:3, e:'Try p = 50: 30% off gives 35, minus the coupon gives 25. Checking: 0.7p − 10 = 25 ✓; p − 40 = 10; 0.7(p − 10) = 28; 0.3p − 10 = 5. The traps by name: p − 40 freezes the percent as flat dollars, 0.7(p − 10) applies the coupon before the discount, 0.3p − 10 keeps the discount instead of the price. (At p = 100, p − 40 coincidentally matches — avoid too-round plug-ins.)' },
      { p:null, q:'If m is an even integer, which of the following must be odd?', c:['3m + 1','3m + 2','m² + 2','5m'], a:0, e:'Plug m = 2: 3m + 1 = 7 (odd) ✓, while 3m + 2 = 8, m² + 2 = 6, and 5m = 10 are all even. The property confirms the plug: any multiple of an even number stays even, so only the +1 breaks parity. Concrete test first, property check second — the pair is faster than abstract reasoning alone.' },
      { p:'A question asks: "If y = x + 3, which expression equals 2y − x in terms of x?" A student begins substituting algebraically, gets tangled, and restarts twice.', q:'How would Plug In Numbers approach it?', c:['Pick x = 2, so y = 5 and 2y − x = 8, then find the choice giving 8','Solve for x in terms of y first, and then substitute back through twice','Test each choice by setting it equal to 2y − x and expanding','Wait to answer until the algebraic substitution comes out clean'], a:0, e:'Concrete numbers end the tangle: x = 2 makes y = 5, so 2y − x = 8. The choice equal to 8 at x = 2 (here, x + 6) wins. Variables in the choices are the M2 signal — the algebra she kept restarting was never required.' },
    ],
    M3:[
      { p:null, q:'A store raises a price by 25%, then takes 20% off the new price. The final price is what percent of the original?', c:['95%','105%','100%','80%'], a:2, e:'Start at 100: up 25% → 125; then 20% off 125 → 100. Exactly 100%. The trap 105% adds the percents (25 − 20 = +5); 95% subtracts them the other way; 80% applies only the discount. Percent changes multiply on shifting bases — the 100 start makes that visible in two quick steps.' },
      { p:null, q:'After a 40% reduction, a coat costs $72. What was the original price?', c:['$100.80','$112','$115.20','$120'], a:3, e:'The $72 is 60% of the original (100 − 40), so the original is 72 ÷ 0.6 = $120. Check: 40% of 120 is 48, and 120 − 48 = 72. The traps: $100.80 multiplies 72 by 1.4 (a percent of the wrong base), $115.20 uses 1.6, and $112 adds a flat $40. Reverse-percent questions always hide a base switch.' },
      { p:null, q:'In a survey, 30% of respondents were students, and 40% of those students were seniors. What percent of all respondents were senior students?', c:['12%','70%','10%','35%'], a:0, e:'Plug 100 respondents: 30 are students, and 40% of those 30 is 12 seniors. 12 out of 100 is 12%. The traps add the percents (70%), subtract them (10%), or average them (35%) — each dodges the "percent OF a percent" multiplication that the 100 makes trivial.' },
      { p:null, q:'A tank\'s volume increases 50% in March and decreases 50% in April. At the end of April, compared to its start-of-March volume, the tank is:', c:['unchanged','25% smaller','50% smaller','25% larger'], a:1, e:'From 100: up 50% → 150; down 50% of 150 → 75. That\'s 25% below the start. "Unchanged" is the canceling illusion (+50 − 50 = 0); "25% larger" flips the direction; "50% smaller" applies April\'s cut to the original base. The second percent acted on 150, not 100 — the compounding is the entire question.' },
      { p:'A problem reads: "A quantity grows by 8%, then by another 8%. By what percent has it grown in total?" A student answers 16% instantly and moves on.', q:'What does Plug In 100 reveal?', c:['Her instant answer is right: consecutive percents add whenever they are equal','16% is close but the true total is 16.64%, because the second 8% acts on 108','She should have answered 1.08, the decimal multiplier','64% — the percents should have been multiplied as 8 × 8'], a:1, e:'From 100: +8% → 108; +8% of 108 → 116.64. Total growth is 16.64%, not 16 — the second increase acted on a bigger base. Small gaps like this are exactly what "which is closest" percent items measure, and the 100-start exposes them in seconds.' },
    ],
    M4:[
      { p:null, q:'For which value of x is (x − 2)(x + 5) = 60?', c:['3','5','10','7'], a:3, e:'Testing beats expanding: x = 7 gives (5)(12) = 60. The quadratic route — expand to x² + 3x − 70 = 0, factor, discard the negative root — costs two minutes and three error chances. The traps: x = 5 gives 30 (half the target), x = 10 gives 120 (double), x = 3 gives 8. Direct multiplication was the whole job.' },
      { p:null, q:'The sum of three consecutive odd integers is 87. What is the largest of the three?', c:['31','29','27','33'], a:0, e:'Test candidates for the largest: if it\'s 31, the trio is 27 + 29 + 31 = 87. Done. The traps are the OTHER members of the same trio — 29 is the middle value (which equals 87 ÷ 3, the classic slip) and 27 the smallest — plus 33, one odd step too far. Testing a choice answers the exact question asked.' },
      { p:null, q:'If 2ᵏ · 2³ = 2⁹, what is the value of k?', c:['3','6','12','27'], a:1, e:'Test k = 6: 2⁶ · 2³ = 2⁹, because exponents add (6 + 3 = 9). The traps run the wrong operations on 9 and 3: dividing them (3), adding them (12), multiplying them (27). If exponent rules feel shaky, testing candidate exponents against the addition check settles it without deriving anything.' },
      { p:null, q:'A rectangle\'s length is 3 more than its width, and its area is 40. What is its width?', c:['4','8','5','10'], a:2, e:'Test w = 5: length 8, area 40, and 8 is indeed 3 more than 5 — both conditions verified in one multiplication. The traps: 8 answers the wrong question (that\'s the length), while 4 and 10 come from the factor pair 4 × 10, right area but wrong gap. Testing checks every constraint at once; the algebra checks them one at a time.' },
      { p:'Facing "x² + 7x = 44, x > 0," a student spends ninety seconds trying factor pairs of 44 in her head, then starts the quadratic formula with a = 1, b = 7, c = −44.', q:'What is the Solve-Without-Algebra observation?', c:['The quadratic formula is the intended and fastest route here','Factor pairs were close; she should have persisted with them','She cannot possibly avoid algebra, because the scenario lists no answer choices','With answer choices on screen, testing them in x² + 7x beats both of her routes'], a:3, e:'On the SAT the choices are sitting there: testing x = 4 gives 16 + 28 = 44 in one line. Ninety seconds of factoring plus a quadratic-formula setup is the "proper math" reflex this strategy retires — the answer bank is part of the problem, so use it.' },
    ],
    M5:[
      { p:null, q:'18.2% of 495 is closest to:', c:['90','9','900','49'], a:0, e:'Round: 18% of 500. One-fifth of 500 is 100, so a bit less — 90 fits. The traps are order-of-magnitude slips: 9 and 900 misplace the decimal, and 49 is roughly 10%. Ballparking exists exactly for this format: the range check IS the answer; no precise multiplication needed.' },
      { p:null, q:'A car travels 297 miles on 9.8 gallons of fuel. Its fuel efficiency is closest to:', c:['3 mpg','30 mpg','45 mpg','300 mpg'], a:1, e:'Round to 300 ÷ 10 = 30 mpg. The traps: 3 and 300 are decimal-shift errors, and 45 splits differences that were never in play. When choices sit an order of magnitude apart, rounding to friendly numbers decides the question in five seconds.' },
      { p:null, q:'A square garden has an area of 175 square feet. Its side length is:', c:['between 8 and 9 feet','between 10 and 11 feet','between 13 and 14 feet','between 17 and 18 feet'], a:2, e:'Bracket with known squares: 13² = 169 and 14² = 196, and 175 sits between them. No square-root button required. The range 10–11 brackets 100–121 (too small), 17–18 belongs to areas near 300, and 8–9 to areas near 70. Squeeze the target between anchors you already know.' },
      { p:null, q:'Of a school\'s 1,214 students, 48.6% ride the bus. Approximately how many students ride the bus?', c:['242','486','728','590'], a:3, e:'48.6% is essentially half, and half of 1,214 is about 607 — the only nearby choice is 590. The bait is 486, the percent\'s own digits dressed up as a count; 242 is roughly 20%, 728 roughly 60%. "Approximately" plus spread-out choices means the half-check does everything.' },
      { p:'A student computes a cone\'s volume and gets 4,710 cubic cm. The cone fits inside a box of about 1,500 cubic cm. She writes 4,710 anyway because "that\'s what the formula gave."', q:'What is the Ballparking discipline she skipped?', c:['The sanity check: nothing outgrows the box that contains it','Re-deriving the cone\'s volume formula from the cylinder\'s formula','Converting the cubic centimeters to liters before answering anything','Recomputing the same formula with more decimal places to be safe'], a:0, e:'A cone inside a ~1,500 cm³ box cannot hold 4,710 cm³ — the answer failed before any formula check. (She likely dropped the ⅓: 4,710 ÷ 3 = 1,570, right at the box\'s size.) Ballparking\'s last job is always this question: does the number fit the picture?' },
    ],
    M6:[
      { p:null, q:'A printer produces 45 pages per minute. How many pages does it produce in 2 hours?', c:['90','5,400','2,700','540'], a:1, e:'Write the units: 45 pages/min × 120 min = 5,400 pages. The trap 90 multiplies by 2 "hours" without converting; 2,700 converts only one of the hours; 540 drops a digit. Setting up the conversion factor FIRST — 120 minutes, not 2 — is the entire safeguard.' },
      { p:null, q:'Fabric costs $12 per yard. What is the cost per foot?', c:['$36','$15','$4','$9'], a:2, e:'Three feet make a yard, so a foot costs one-third of a yard\'s price: $4. The bait is $36 — multiplying instead of dividing, the classic inverted conversion. Run the direction check first: a foot is SMALLER than a yard, so its price must be smaller. Direction, then divide.' },
      { p:null, q:'A recipe uses 250 grams of flour per batch. How many full batches can a 3-kilogram bag supply?', c:['83','120','8','12'], a:3, e:'Convert first: 3 kg = 3,000 g; then 3,000 ÷ 250 = 12 batches. The traps: 120 slips a decimal in the division, 8 converts to 2,000 g, and 83 divides the wrong way around. Kilograms to grams BEFORE any arithmetic — the conversion is the question.' },
      { p:null, q:'A cyclist rides at 24 km/h. How far does she travel in 40 minutes?', c:['16 km','960 km','9.6 km','36 km'], a:0, e:'Units first: 40 minutes is ⅔ of an hour, and 24 × ⅔ = 16 km. The monster 960 multiplies km/h by minutes directly (mixed units); 9.6 treats 40 minutes as 0.4 hours; 36 runs the time ratio upside-down. Convert minutes to hours BEFORE touching the speed.' },
      { p:'A problem gives a pump\'s rate in liters per minute and asks how many hours it needs to fill a tank measured in kiloliters. A student multiplies the two given numbers immediately and picks the choice that matches.', q:'What does Units-First predict about her answer?', c:['It is probably right, since matching one of the offered choices confirms her setup','Likely a planted trap: unconverted numbers usually match a wrong choice by design','It depends on whether the pump\'s stated rate was physically realistic','It is wrong only if the tank\'s shape happened to be irregular'], a:1, e:'Two conversions were owed — minutes to hours and liters to kiloliters — and skipping both still lands on a choice, because the test PLANTS the unconverted result. A matching choice is not confirmation; it\'s bait. Write the unit chain first, and the traps have nowhere to hide.' },
    ],
    M7:[
      { p:null, q:'Maya buys 3 notebooks at $4.50 each and pays with a $20 bill. How much change does she receive?', c:['$13.50','$15.50','$6.50','$7.50'], a:2, e:'Total cost 3 × 4.50 = 13.50; change 20 − 13.50 = 6.50. The planted answers: $13.50 is the COST — the classic stop-one-step-early trap; $15.50 subtracts only one notebook; $7.50 is a borrow slip in the subtraction. Your answer appearing among the choices proves nothing — the mistakes are printed there too.' },
      { p:null, q:'If 5x − 3 = 22, what is the value of 2x + 1?', c:['5','25','10','11'], a:3, e:'x = 5, so 2x + 1 = 11. The planted 5 is x itself — the answer to a question that wasn\'t asked, waiting for anyone who stops early. 10 forgets the +1; 25 echoes 5x. Re-reading the ask ("what is 2x + 1?") is the last line of defense, and the cheapest.' },
      { p:null, q:'A class\'s average over 5 tests is 84. After a sixth test, the average rises to 85. What was the sixth test\'s score?', c:['90','85','86','510'], a:0, e:'Work in totals, not averages: five tests total 420; six tests at an 85 average must total 510; the sixth score is 510 − 420 = 90. The traps: 85 repeats the new average, 86 follows the "just one more than 85" intuition, and 510 stops at the six-test total. Average problems hide totals — the traps are the intermediate numbers en route.' },
      { p:null, q:'A circle has diameter 10. What is its area?', c:['100π','25π','10π','50π'], a:1, e:'Radius is half the diameter: r = 5, so area = 25π. The headline trap is 100π — the diameter used as the radius, the single most-planted circle mistake on the test. 10π is the circumference (2πr) wearing an area costume; 50π halves the wrong quantity. Naming the trap in advance is what stops you from re-inventing it under pressure.' },
      { p:'While reviewing, a student notices her wrong math answers are usually one of the printed choices, not random numbers. She concludes the test "reads her mind."', q:'What does Trap Answer Arithmetic actually explain?', c:['Coincidence: any number is likely to appear among four choices','The test adapts its choices to each student\'s past errors in real time','Choices are built from common mistakes, so errors land on printed traps','Her calculator introduces rounding that happens to match the choices'], a:2, e:'No mind-reading required: the wrong choices are manufactured from the most common slips — stopping early, wrong base, dropped negative — so a predictable mistake always finds a printed home. The flip side is the defense: know the likely slip for each problem type, and check your answer against it before submitting.' },
    ],
    M8:[
      { p:null, q:'If j is an odd integer and k is an even integer, which of the following must be odd?', c:['jk','2j + k','4k','j + k'], a:3, e:'Properties, not computation: odd + even = odd, so j + k must be odd. Anything carrying an even factor (jk, 4k) stays even, and 2j + k stacks two evens. Confirm with j = 3, k = 2: the choices give 6, 8, 8, and 5. The parity rules decided it before the arithmetic started.' },
      { p:null, q:'The product of two integers is negative. Which of the following must be true?', c:['Exactly one of the two integers is negative','Both of the integers are negative','Both integers have absolute value greater than 1','The sum of the two integers is negative'], a:0, e:'Sign rules: a negative product requires opposite signs — exactly one negative. Both negative would give a positive product; the sum could go either way depending on magnitudes; and absolute values are unconstrained (−1 × 3 = −3). One property replaces every possible calculation.' },
      { p:null, q:'Which number is divisible by 3?', c:['4,213','4,617','5,036','7,121'], a:1, e:'The divisibility property: a number is divisible by 3 exactly when its digit sum is. 4 + 6 + 1 + 7 = 18 — divisible. The others sum to 10, 14, and 11. Long division on four candidates is the procedure; one line of digit sums is the property.' },
      { p:null, q:'If n is a positive integer, the expression (−1)ⁿ⁺¹ · n² is negative when:', c:['n is prime','n is odd','n is even','n is a perfect square'], a:2, e:'Track only the sign: n² is always positive, so the sign comes from (−1)ⁿ⁺¹. Even n makes n + 1 odd, and −1 to an odd power is −1: negative. Odd n gives +1: positive. "Prime" and "perfect square" are scope decoys — parity is the only property the expression consults.' },
      { p:'A question asks which of four expressions "must be even for all integers n." A student begins expanding each expression algebraically to check, budgeting several minutes.', q:'What is the Properties-Over-Procedures alternative?', c:['Expand each expression faster by memorizing more algebraic identities','Graph all four expressions and visually inspect their outputs','Skip the question entirely, since "must be" items cannot be proven quickly','Apply parity rules, then test one value to confirm'], a:3, e:'"Must be" plus integer conditions is the property signal: parity rules (even × anything = even; odd + odd = even) classify each expression in seconds, and one plugged value verifies the work. Expansion answers a harder question than the test asked.' },
    ],
    M9:[
      { p:null, q:'A right triangle has legs of length 9 and 12. What is the length of its hypotenuse?', c:['15','21','12.5','108'], a:0, e:'Estimate before computing: the hypotenuse must beat the longest leg (12) and undercut the legs\' sum (21). That kills 12.5 and 21 immediately, and 108 is an area-sized number in a length slot. The 3-4-5 family confirms: 9-12-15. The bounds did most of the work.' },
      { p:null, q:'A circle is inscribed in a square of side 8. The area inside the square but outside the circle is closest to:', c:['50','14','28','64'], a:1, e:'The circle (r = 4, area ≈ 50) fills most of the square (64), so the leftover corners are small: 64 − 50 ≈ 14. The choices 50 and 64 are the circle\'s and square\'s own areas — planted intermediates — and 28 would mean the corners take nearly half the square, which the mental picture rules out.' },
      { p:null, q:'On a coordinate grid, point A is at (1, 2) and point B is at (7, 10). What is the distance from A to B?', c:['14','8','10','6'], a:2, e:'Legs of 6 (across) and 8 (up) make a right triangle: 6-8-10. Even before spotting the triple, bounds decide it — the distance must beat the longer leg (8) and undercut the walk-around path (14). The traps: 6 and 8 are the legs posing as the answer, and 14 is the taxicab route.' },
      { p:null, q:'A triangle has vertices at (0, 0), (6, 0), and (6, 8). What is its area?', c:['48','14','10','24'], a:3, e:'Base 6 along the x-axis, height 8 up the right side: area = ½ × 6 × 8 = 24. The headline trap is 48 — the bounding rectangle\'s area, the forgotten ½. And 10 is the hypotenuse\'s length wandering into an area slot; 14 adds the legs. Sketch the three points and see half a rectangle.' },
      { p:'A described figure shows a slanted segment labeled x inside a unit square, clearly shorter than the square\'s diagonal. A student\'s computation yields x = 1.9, and she submits it.', q:'What does Geometry Estimation flag?', c:['Nothing in a unit square exceeds the ~1.41 diagonal, so 1.9 fails the picture','Slanted segments inside squares cannot take irrational lengths as answers','Computations performed inside unit squares always yield values below one','The figure must be certified as drawn to scale before estimation applies'], a:0, e:'The longest segment a unit square can contain is its diagonal, √2 ≈ 1.41. A result of 1.9 is impossible before checking any algebra — the picture sets hard bounds. Estimation\'s role in geometry is exactly this: bounds first, formulas second.' },
    ],
    M10:[
